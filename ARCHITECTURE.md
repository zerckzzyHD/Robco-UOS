...
│   ├── ui.js           ~46KB  Core UI lifecycle, appendToChat, loadUI, updateMath
...
├── tests/
│   ├── check-persistence.ps1   28KB    689-test pre-commit audit
│   ├── check-persistence.js    36KB    689-test Node runner (parity with .ps1)
...
7. js/ui.js         → defines: AudioSettings, appendToChat, loadUI, updateMath, etc.
8. js/api.js        → defines: autoImportState, transmitMessage, fetchAuthorizedModels
9. js/cloud.js      → loaded as <script type="module"> (ES import from Firebase CDN)
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (only if a served file is staged; skipped for doc/CI/test-only commits), then the 689-test persistence audit
