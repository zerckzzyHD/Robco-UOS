...
├── tests/
│   ├── check-persistence.ps1   28KB    1420-test pre-commit audit
│   ├── check-persistence.js    36KB    1420-test Node runner (parity with .ps1)
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (only if a served file is staged; skipped for doc/CI/test-only commits), then the 1420-test persistence audit
