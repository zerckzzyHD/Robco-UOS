...
├── tests/
│   ├── robco-diagnostics.js    36KB    3258-test Node runner (the single canonical gate audit)
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (only if a served file is staged; skipped for doc/CI/test-only commits), then the 3258-test persistence audit
