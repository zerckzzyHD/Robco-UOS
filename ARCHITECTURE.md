...
├── tests/
│   ├── check-persistence.ps1   28KB    757-test pre-commit audit
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (only if a served file is staged; skipped for doc/CI/test-only commits), then the 757-test persistence audit
