...
> **This list is GENERATED (Protocol 52, `rules/docs-and-library.md`) — never hand-edit the
> block below.** `npm run architecture-toc` derives it from this file's own `## ` headings and
> their preceding `<a id="…">` anchors, in reading order; the gate's `architecture-toc:check`
> step fails the build if the committed list drifts from what the generator would currently
> produce. It replaces a hand-typed list that had already drifted for real once (previously 19
> entries, 20 headings out of date, fixed at R10 Step 3). The link text below is each heading's
> FULL text, verbatim — no shortening — because that is the only zero-false-positive source
> (Protocol 3).

<!-- TOC:BEGIN — generated by scripts/generate-architecture-toc.js (`npm run architecture-toc`); do not hand-edit, see Protocol 52 in rules/docs-and-library.md -->
...
│   │   ├── ui-core.js      Core UI lifecycle hub — AudioSettings, appendToChat, loadUI, updateMath, window.onload boot orchestrator (2.8.5 U-A1 split spine) ⭐⭐ Also owns LINK STATE (Suite 264): `_assessLink()` is a PURE, three-valued decision over navigator.onLine and a reachability probe — disconnected / live / unreachable / UNKNOWN. ⛔⛤ The probe uses a UNIQUE, cache-missing URL because sw.js answers every same-origin request `caches.match(request) || fetch(request)` — a probe of any precached path is answered BY THE SERVICE WORKER from disk and reports a dead host as alive, which is a check reading a CLAIM instead of an EFFECT inside the feature written to end that confusion (the `cache: no-store` hint does NOT prevent it and is not what makes this work). ⚠ Any HTTP response counts as reached, 404 included — gating on `res.ok` would call a live host unreachable. ⭐ The unreachable wording names BOTH the host and the route to it and commits to NEITHER: the browser cannot tell them apart, and printing a guess is what sent the owner to check the wrong system. ⛔ A healthy link renders nothing, and "a newer version exists on the host" is deliberately NOT collapsed in here — the service-worker update prompt owns that (Protocol 22)
...
│   ├── queue-filter-check.mjs  Drives the dev-server /queue page's PROJECT FILTER in a real browser and asserts the numbers a reader SEES. ⛔⛤ The filter shipped with the band headline numbers ignoring it — only a caption beneath them responded — so the largest number on each row answered a different question from the one just asked; found by the owner on his phone. ⚠ The Node suite asserts the DATA the page hands its script, which is necessary and is NOT the same as the script working: a check that reads a program's input is not a check that the program runs. This clicks every project pill and compares each band's rendered figure against the server-computed (band × project) map the page published, asserts every pill equals the sum of the bands selecting it produces (the residual the first fix introduced an hour later), asserts All restores exactly, and fails on any page error. ⭐ Needs NO server — the page is one self-contained document opened over file://. Proven red-then-green by replaying the original defect. SKIPS with its reason on a checkout without the private planning tree
...
│   ├── board-axes.js          The ONE derivation of the board's two NEW axes — HORIZON (should this be counted yet: BLOCKS-WORK-NOW · NEXT · SOMEDAY-IF, read from each item's own `accept` block) and the DECIDE/DO split behind the /queue "need you" tiles. ⛔⛔ Its load-bearing rule is the owner's: a SOMEDAY-IF item MUST NOT APPEAR IN A BACKLOG COUNT AT ALL — `excludeSomeday()` is that rule in one place, and every total on /queue goes through it. ⭐ THREE-VALUED AT TWO LEVELS, which is the whole point on a board where 405 of 411 items carry no horizon yet: per item a value · UNSET · UNPARSEABLE (an unset horizon is NEVER defaulted — reading it as BLOCKS-WORK-NOW inflates the live count, reading it as SOMEDAY-IF hides real work), and per axis observable-with-counts · UNOBSERVABLE-with-a-reason, because "every item is unset" and "the field could not be read" print the same reassuring big number while meaning opposite things. ⛔ NO GRAMMAR IS RETYPED HERE: the accept-block parser and the horizon vocabulary are INJECTED from the archive's own `!PLANNING/tools/item-format-check.cjs` — the gate that refuses a malformed block — the mirror of that check importing this repo's parseQueue, so a horizon typo cannot mean one thing to the gate and another to the page. Pure functions, no I/O; resolution is planning-paths.js's job (Suite 270). ⭐ The PROJECT axis reads the graph's per-item row and, ONLY when the graph has no row for the item, the item's own block `project:` through the same injected grammar (basis `BLOCK`, `projectOfBody`); where both homes speak the graph still wins — precedence between them is a ruling, not a reader fix (2026-09-23, 76 → 0 UNSET on the live board; Suite 270.21). ⭐⭐ And when the archive's ONE resolver is reachable (`!PLANNING/tools/item-resolver.cjs`, loaded by `planning-paths.loadResolver()`; the same function the checkpoint preflight, the ranking tool and the format gate import), BOTH readers take its per-item value, basis and conflicts as final and apply no precedence or demotion of their own — the owner's D2-A rule (a quoted reading, then the block, then the graph) lives there, once. `report-view` then prints a `RESOLVER-COUNTS v1` line built from the page's OWN tallies, so agreement with the other readers is measured, not asserted. Without the resolver the two-source readers run as before (Suite 270.22)
...
│   ├── atomic-write.js         WF12 — the ONE way a tool here replaces a durable file: write a sibling temp, fsync, rename over the target, so the destination is never truncated before the replacement bytes exist. Write loop refuses a zero-progress short write (WF15). A DELIBERATE reimplementation of the archive's `!PLANNING/tools/atomic-write.cjs`, never an import — a cross-repo require would make this public repo depend on a private sibling checkout being present. Behaviourally guarded by Suite 259
...
│   ├── dev-server.js           One-step start/stop/status for the local Vite dev server (`npm run dev:start` / `dev:stop` / `dev:status`) — spawns Vite DETACHED so it survives the SSH tab closing, is idempotent (no-ops when already up, REFUSES rather than stacking a second server on a busy port), re-ensures the `tailscale serve` tailnet proxy, and REPORTS the checked-out branch with a loud warning when it is not `dev` — it never switches or stashes anything. Not a service, but: it comes back after a REBOOT once the logon trigger is installed (`dev-autostart.js`), and this machine's SLEEP (S0 idle) keeps the process alive — only the network drops (Suite 249.8). What ends it: hibernate, or a hand stop; what kills the URL while it lives: Tailscale dropping. (Corrected 2026-09-03 — it used to claim no reboot or sleep survival, both false)
...
│   ├── home-view.js           Renders the dev server's landing page at `/` (`/home` redirects there since 2026-09-03; the app itself moved under `/terminal/`) — one phone-readable screen that reaches every destination. ⛔ Holds NO addresses of its own (Suite 260.3): every destination is passed in by the route, so one reviewable place governs them. ⭐ Its rule is that a destination is linked ONLY when it was verified by CONTENT — a dev server answers unknown paths with the app's index page at the same status and byte length, so "it returned 200" proves nothing. Destinations that do not exist are named as not-built in PLAIN TEXT, never as anchors (Suite 260.1), because a dead link is the page lying. ⛔ Those absences are HANDED IN like every other destination, never held as a literal here (Suite 262.8) — when they were a constant inside the renderer, two pages were built and the page went on announcing that neither existed, which building them could not possibly correct. That failure is the silent direction: a dead link at least fails when tapped, while "not built yet" about something that IS built offers nothing to tap and is simply believed. Reuses report-view.js's `page()` shell and palette (Protocol 22) rather than growing a second look ⭐ Owns the shared age vocabulary the status page imports rather than restates — `ago()`, `elapsed()` and `assessAge()` (fresh / ageing / stale) — so the tile and the page it links to cannot develop two opinions about what "stale" means. ⛔ Its status tile no longer calls itself "Live": that title sat above the words "Snapshot taken 4 days ago", asserting a property the code cannot honour with its own correction printed underneath it, where a correction is read second if at all (Suite 262.9) ⛔ Its build-board tile reports the same currency rather than the board file's mtime — "Updated 44 minutes ago" was true and was the shape of reassurance while 59 items were missing (Suite 262.20). Three-valued, handed in like everything else here: the renderer measures nothing
...
│   ├── worktree-state.js       READ-ONLY measurement of one git worktree (HEAD, upstream parity, tracked/untracked counted separately, index.lock evidence, tree motion, Claude/Codex sessions in the repo). Every git call is `--no-optional-locks` — a preflight must not strand the lock it reports on. Paths are canonicalised (`realpathSync.native`) before any "same directory?" comparison — the record's cwd and git's toplevel come from different producers and can spell one place two ways (8.3 short names, junctions), and a lexical compare answers "no writers" with confidence. Shared by the two Codex hooks below (Suite 266)
│   ├── worktree-preflight.js   Codex SETUP hook (one-line pointer in the vendor box): prints the repo's state at worktree creation — reports, NEVER refuses, exit 0 always
│   ├── worktree-teardown-guard.js Codex CLEANUP hook: refuses (fail-closed, files NAMED) to discard uncommitted work — exit 1 dirty / 2 unobservable — and copies the work out to `~/.robco/worktree-rescue/` first, because whether Codex honours the exit code is UNVERIFIED (its header says ADVISORY until measured)
│   ├── queue-view.js           Parses QUEUE.md into a private, phone-readable HTML view (`npm run queue-view`; no-ops without the archive)
│   ├── roadmap-generate.js    Bands QUEUE.md's items into a generated ROADMAP.md in the private tree (`npm run roadmap`; `--check` via `npm run roadmap:check` exits 1 when the board is MISSING, BLIND, unverifiable, or STALE against QUEUE.md's sha256). Reuses queue-view.js's parser + vocabulary (never a second parser); a structural parse gap renders the WHOLE doc BLIND rather than a partial list, and the generator itself always exits 0 — it is a reporter, never a blocker ⭐⭐ `--check` also proves the board IS WHAT THIS GENERATOR PRODUCES — it rebuilds in memory and compares byte for byte with the `App repo HEAD` stamp normalised out (Suite 248.7i). ⛔ That gap was documented here as PERMANENT: a fingerprint answers "built from the current source?" and can never answer "is this what the generator makes?", so a board rendered by an older script, or hand-edited below the provenance block, passed every check there was — the two ways a board is wrong while its source stands perfectly still, which is the harder direction to notice because nothing moved to prompt a look. The old ceiling was right about the STAMP (it changes on every unrelated commit — the 247.10 trap) and wrong to generalise from it to the DOCUMENT: the stamp is one line with one anchored pattern beside its emitter, and holding it out leaves something this generator's own contract declares deterministic. Suite 248.7h is unchanged and now guards the byte-compare instead of a fingerprint. ⭐ It also exports `boardCurrency()` — the ONE definition of "does this board match this queue", so the pages that DISPLAY the board ask the same question the gate does (the full rebuild-and-compare is a gate cost, not a page-load one; the viewers ask the cheap fingerprint half and SAY that is the question they asked) ⭐⭐⭐ AND `npm run roadmap` REPORTS THE DRIFT IT JUST CLOSED (Suite 248.7k) — the delta, the newly-listed IDs, and the limit of that naming (the backlog band is a count, so an item landing there moves the totals and cannot be named). ⛔⛤ This is the answer to the `roadmap && roadmap:check` tautology, and it is deliberately NOT a stricter check: after a successful regenerate the board genuinely IS current, so every honest predicate must answer yes there — one that answered no would be a false positive, not a stronger gate. The tautology is a property of the ORDERING. ⭐ The harm was never that a check passed; it is that the drift was DESTROYED BEFORE ANYONE MEASURED IT, leaving no trace that 59 items had been missing for days. Reporting it from the command that closes it makes the ordering stop being load-bearing — a session that tidies the ritual back into one line can no longer lose the measurement
│   ├── dev-autostart.js       Installs/removes a per-user LOGON trigger that starts the dev server (`npm run dev:autostart` / `dev:autostart:off` / `dev:autostart:status`). ⛔ One trigger at logon — not a timer, not a supervisor, no restart loop: nothing resurrects a server you stopped on purpose. Uses the Windows per-user Startup folder rather than Task Scheduler, because `schtasks /Create` needs elevation here (measured: every `/RU` variant returns Access denied) and this must acquire no authority it did not have. It cannot fight a hand-started server — its action is `dev-server.js start`, which already refuses a port it did not open. Removal is one command, by design
│   ├── control-state.js       READ-ONLY resolver for the operational state directory that lives OUTSIDE this repo. ⛔ Its location is CONFIGURED (`ROBCO_CONTROL_STATE`) with no fallback path written down anywhere here — this repo is PUBLIC, and the layout of a private directory should not be discoverable from it; a hardcoded default would publish that permanently to save one environment variable. Unconfigured is a normal outcome, and every page built on it says so rather than rendering an empty success. ⛔ Exposes no write path of any kind — the logs it reads are append-only and hash-chained, so a write would break the verifiability of every earlier record. ⛔⛔ Reads a log only as a BOUNDED TAIL by byte offset: the set is far too large to read whole, so a whole-file read per request could take a personal dev server down. There is deliberately no whole-file mode to reach for ⭐ Also reports the most recent write ANYWHERE in that directory (`newestWrite()`) — a TIME and never a name, so no path through it can put a private filename on a page. That one datum is what lets a viewer tell a switched-off machine from a live one whose producer has died, which a snapshot age alone cannot do (Suite 262.18)
│   ├── status-view.js         Renders the generated operational snapshot as one phone-readable page. ⛔ The snapshot is produced on a schedule, so reading it per request makes the READ fresh, not the DATA — the page therefore LEADS with how old the data is, in words, and says so before any status. Prints every field whether or not it has a value: a missing row and a healthy row look identical once the row is gone. Where the source states its own inability to measure, its reason is carried through rather than replaced. ⭐ COMPLETENESS IS DERIVED, NOT ENUMERATED (Suites 262.6/262.7): the fields it opens in detail are declared once, and every OTHER top-level field the snapshot carries is listed by NAME beneath them — computed from the snapshot in hand, so a field added upstream can never become invisible here. It had been detailing nine of twenty-four while claiming to print them all. ⛔ The remainder is names-only by construction — the only value strings it can emit are `present` and `UNOBSERVABLE` — because several of those blocks record filesystem locations, and a generic "render whatever is left" would publish them and keep doing so for every field added later ⭐⭐ THE AGE HAS THREE TIERS, and the top one is the page's HEADLINE (Suite 262.16): a stopped source is stated above enforcement and above the findings, everything below it is restated "as it was then", and the verdict block carries its own as-of stamp. It was one boolean at fifteen minutes, so sixteen minutes and FOUR DAYS printed the identical "worth double-checking" sentence — measured 2026-09-01 against a snapshot whose producer had emitted nothing since 2026-08-28, which is why a freeze sat unnoticed for three days in front of a reader who was looking at the number. ⛔ It still decides NOTHING about the system: "this page has heard nothing" is a fact about its own input, and the switched-off-versus-dead-producer split is read from the directory measurement handed in by the route, never inferred (Suites 262.17/262.19)
│   ├── ledger-view.js         Renders a bounded window onto the append-only logs, and says on the page that it is a window — "the log contains no errors" and "the end of the log contains no errors" are very different claims, and the second is the one this can support. Never reorders or de-duplicates: the order is the evidence. An unparsable record is shown raw rather than skipped, because a skipped record is an invisible gap in something whose whole value is having none
│   ├── report-view.js         Renders a PRIVATE markdown report into one self-contained, phone-readable HTML page for the dev server's `/reports` route, and the build board as its own page at `/queue` (owner ruling 2026-09-03: one path per thing — the two used to share `/reports` behind a jump menu). ⛔ A renderer only — it holds no report content and has no filesystem write path at all: markdown in, HTML string out, straight into a response body. The reports live OUTSIDE this public repo (resolved by planning-paths.js) and are read at request time, so a rendered report never exists as a file for a commit or a publish walk to find. Reuses queue-view.js's `mdToHtml` (Protocol 22) rather than growing a second markdown renderer ⭐⭐ THE BOARD SECTION LEADS WITH CURRENCY, NOT WITH A REBUILD TIME (Suite 249.10g). It used to print "N items on the board. Rebuilt <time> — read fresh from the file every time this page loads, never cached" — every word true, and none of it the question being asked: it reports when the FILE was written and how fresh the READ was, and the reader hears "this is the current picture". ⛔⛔ Measured 2026-09-01: the board was 59 items stale (307 rendered against 366 live) and this page said exactly that, in that tone, for days. It now compares the board's recorded source fingerprint against the live queue it is already reading for the honesty tile, and states which of the three answers it got — matches / OUT OF DATE / could not be established — with the third kept distinct, since a board nobody could check is not a board that is fine. ⚠ It names the question it asked: the stronger rebuild-and-compare belongs to `npm run roadmap:check`, not to a page load ⭐⭐⭐ REBUILT 2026-09-08 (owner: "match how the queue actually works now"): with a readable queue the page renders the QUEUE ITSELF at the ref — every status in the shared vocabulary is a band (`To-do` under its own name; "Backlog" was the board document's local rename and never a state), every item is one row (id · title · chips: landed-marker / spec / horizon·basis / project), and the ⬜ mass is finally openable instead of a count with no rows. A row expands to its Done-when clause (the generator's own summariser, imported), its LIVE blocker-graph edges as ATTRIBUTES with their basis (⛔ never a drawn cascade — the edges were recorded per-finding, not as a dependency survey; owner ruling 2026-09-08), and lazily fetches its full body from `/queue/item/<id>` (same ref-read source; `?frag=1` returns the bare article for the row's fetch). The live run's landed-state heading markers ([MERGED-AWAITING-CONFIRMATION] / [MERGED-NOT-DEPLOYED] / [ADVANCED-NOT-CLOSED]) are first-class: a watch section above the bands plus a tile, counted over the whole queue with the population stated. The board-scrape rendering survives ONLY as the degraded path when the queue itself is unreadable; a stale ROADMAP.md is still named stale, with the page stating it does not depend on it (Suites 249.10g, 273)
│   ├── projection-view.js     Runs the EXTERNAL read-only control-plane projection renderer for the dev server's `/view` route (`/view.json`, `/view.txt`) — a fresh child process per request, stdout into the response body. ⛔ Configured ONLY (`ROBCO_VIEW_RENDERER`), no default location: the renderer lives in a private tree whose layout this public repo must not disclose (same rule as `control-state.js`). No write path; unconfigured is a plain-text 404, never the app (Suite 265.3)
│   ├── sw-killswitch.js       The service-worker KILL SWITCH the dev server serves at the tailnet origin's ROOT `/sw.js` — a worker with NO fetch handler that clears every cache and unregisters itself. Needed because the app used to live at the root and registered a cache-first worker at scope `/`; the app is under `/terminal/` now (its real worker is `/terminal/sw.js`, untouched) and a phone still holding the old registration would otherwise render the stale root shell over the landing page forever. Dev-only; never precached, never bumped, never shipped (Suite 265.4)
│   ├── dev-host.js           The ONE resolver for the dev origin's tailnet hostname, and the reason it is no longer in tracked source. ⛔⛤ This machine's `*.ts.net` MagicDNS name sat in TRACKED source on this PUBLIC remote from 2026-08-24 (`838c839`, "name the tailnet host in Vite's allowedHosts") to 2026-09-08 — a functional `allowedHosts` value in `vite.config.mjs`, twice more in a `dev-env-marker.js` comment, and the machine\user form in `CLAUDE.md`. Found while deleting three `overnight/*` branches that carried it in an acceptance receipt; the branches were the small half and `dev` was the large one. ⭐ Owner ruling 2026-09-08, option (a): remove the CLASS, not the instance — the value moves OUT of tracked source and the tracked file carries a placeholder naming where the real one lives, so rotating the tailnet name or adding a second dev origin costs one untracked file rather than a commit. Resolution order is `ROBCO_DEV_HOST` (comma-separated; the CI/test door, matching planning-paths.js's `ROBCO_*` convention) then `dev-host.local.json` at the repo root (gitignored, same DNA as `.mcp.json` — absent by design on a public clone). ⚠ THE FILE IS THE PRIMARY AND THE ENV IS THE OVERRIDE, which is the opposite of the usual instinct, because the dev server is started by a LOGON TRIGGER: a value living only in a shell environment is absent from that process, so an env-only design would work by hand and fail silently at every reboot — and surviving a reboot is this server's stated acceptance test. ⛔ THREE-VALUED WITH NO FALLBACK HOST: `{ok,hosts,source}` or `{ok:false,why}`, and `vite.config.mjs` REFUSES to start on `serve` rather than binding an empty host list, because a wrong `allowedHosts` does not fail loudly — Vite serves and the phone gets a "Blocked request" page that reads like a network fault. The refusal is scoped by the `command` Vite hands `defineConfig`, never by sniffing `process.argv` (bare `vite` IS serve and carries no such token — the argv version silently disarmed itself, caught by its own red control). A wildcard is refused too, so an untracked file cannot re-make the DNS-rebinding decision where nobody reviews it. ⛔ History is NOT rewritten: the value stays in past commits and every clone, deliberately — the class is closed going forward. Severity stated rather than assumed: a MagicDNS name resolves only inside a private tailnet and is never funnelled, so this is hygiene, not an incident (Suite 274)
│   ├── dev-env-marker.js      The ONE definition of the environment marker the dev server stamps into the HTML it serves (`<meta name="robco-env" content="staging">`) — the same tag `cf-staging-build.mjs` injects for Cloudflare staging, so the two origins agree rather than one being special. It is what makes the Diagnostic Shell exist on the tailnet dev origin: `_isStagingEnv()` (ui-core.js) previously recognised only that meta, an unset global, and a hostname list (`localhost`/`127.0.0.1`/`*.pages.dev`) written before this origin existed — so the shell mounted at 127.0.0.1 and was invisible over the tailnet from byte-identical HTML. A marker EMITTED BY THE SERVER, not one more trusted hostname shipped in public app code: the predicate becomes "served by a dev server" (positive, host-agnostic) rather than "answering to a name on a list". ⛔ Injected by an `apply:'serve'` Vite plugin only — the tracked `index.html` carries no marker, so the PUBLIC site inherits no staging signal (Suite 249.12)
│   ├── dev-pwa-identity.js    The DEV app’s own installable identity — a manifest DERIVED from the real `manifest.json` (only `name`, `short_name` and `icons` overridden; `start_url`/`scope`/shortcuts/colours inherited, never forked) plus a generated amber SVG icon, both served under `/terminal/` by an `apply:'serve'` route. ⛔ Why it exists: the published app and the dev app are separate origins, so separate installs — but they read the SAME manifest, so both installed as “RobCo U.O.S.” with one icon, and opening the wrong one makes every conclusion after it wrong. Now “RobCo DEV”, amber. ⚠ `start_url`/`scope` are deliberately untouched: they are `./`, relative to the manifest’s own URL, so they already resolve to `/terminal/` — verified against Chrome’s own manifest parser (zero errors, zero installability errors). The tracked `manifest.json` is unchanged, so production is byte-identical (Suite 249.13)
...
│   ├── backup-nudge.js         Protocol 48 pre-push nudge — flags when the local-only artifacts (library/, planning/, agent memory) have drifted from the private archive backup. ⭐⭐ THE PREDICATE IS **CONTENT EQUIVALENCE** — every watched artifact is compared byte-for-byte against the archive tree at a PINNED sha (Suites 237.9–237.14). ⛔⛤ It used to compare local **mtimes** against the archive’s **last commit timestamp**, so ANY commit there discharged it whether or not it backed anything up; measured 2026-09-01, three artifacts were unbacked while it reported nothing owed. ⛔⛔ "Check presence" would have been equally silent — one of the three was PRESENT and 144 bytes stale. ⭐ It is also **never silent**: BACKED UP / NOT BACKED UP / UNOBSERVABLE / NOT CONFIGURED, with the last two kept distinct because an absent archive (normal for a public clone) and an unreadable one (a failure to measure) are different facts. ⛔ Fail-safe polarity unchanged (Protocol 33) — it informs, it never gates. Archive reads are index-free at a pinned sha, because that repo has a live writer
│   ├── backup-nudge.eol.test.js  BD16 driver — proves the nudge's LF fallback forgives line endings and NOTHING else. E1 is the control (raw hashes must genuinely differ); E3/E3b pin that real content changes and truncations are still caught; B1 pins binary exclusion (429 PNGs in scope); I1 pins that importing backup-nudge.js is inert.
│   └── rollback.sh             Protocol 16 one-command hotfix rollback
├── CHANGELOG.md        Full version history
├── SPINE.md            The shared facts of the RobCo project, written ONCE — repo map with measured visibility, the doctrine sentences, where every canonical file lives. Every CLAUDE.md / AGENTS.md in the RobCo repos points here (owner's DOC1 ruling; placed 2026-09-03)
...
Guarded by Suite 158.21–158.27 (Node runner). Cache r118.
...
Guarded by Suite 158.28–158.30 (Node runner); this unit also
...
Two follow-up fixes to the DO-N/mobile-immersion bezel nav: (1)
...
Owner-report fix — uniform bezel nav keycap width (Suite 158.33–158.34).
...
from persistence. Guarded by Suite 162 (Node runner, including a Node
...
Guarded by Suite 192 (Node runner): the three-board split, the button/mirror
...
Guarded by Suite 181 (Node runner): id/handler preservation, the `stat_rads`
...
Guarded by Suite 182 (Node runner): behavioral proofs (via a Node `vm` sandbox executing
...
Guarded by Suite 183 (Node runner): the `.panel.bay-board` margin-top rule and its scoping
...
Protocol 13-regression-tested (Suite 221, expanded to cover both follow-ups; Suite 12).
...
Guarded by Suite 185 (Node runner): the id/handler-preservation contract, the caps/weight
...
Guarded by Suite 189 (Node runner): the deleted `_mapAbbrev` table, the SVG node-map's
...
Guarded by Suite 208 (Node runner): each moment's trigger, the zero-write
...
Guarded by Suite 209 (Node runner): the token scale (base + mobile-tuned
...
| `!PLANNING/ROADMAP.md`          | **GENERATED** | ⛔ **NEVER hand-edited**                            | A banded projection of `QUEUE.md`, fingerprinted, fail-closed — in-motion bands listed in full, **backlog as a count** |
...
⭐ **The BACKLOG band is a COUNT, not a list (QR3, 2026-08-13).** The restructure plan said so from the start — _"BACKLOG is a count, not a list, and that is the whole design"_ — and the first generator shipped it as a full list of 148 items anyway, roughly 60% of the document. That is not a cosmetic drift: it is the generated board reproducing the exact unreadability its own parent item was filed to end. The queue got lean; the view of it did not. The band now renders as `## ⬜ Backlog — N items` plus a one-line pointer back to `QUEUE.md`, taking the board from 247 lines to 100. Bands describing work **in motion** (ready, active, attention, deferred, parked) stay full lists, because naming their items is their whole value; backlog's value is its **size**.

⚠ **Hiding items behind a number does not weaken drop detection**, which is worth stating because it looks like it should. Nothing in the rendering decides what _exists_: `extraction-regression` still compares the parser against a raw scan of the source, the header still prints the ID-bearing total, and the band counts still have to add up to it. A silently dropped backlog item blinds the whole board exactly as before — it just no longer costs 148 lines to say so.

⛔ **`UNCLASSIFIED` is explicitly not covered by that rule** and stays listed in full, never truncated, never guessed at. It is the single most dangerous place a "render this band as a number instead" change could be applied next, so Suite 248.8 asserts both directions from one fixture — backlog counted **and** unclassified listed in full — rather than locking only the easy half.

The `Backlog` label is a deliberate **board-local** override: the shared `STATUSES` vocabulary calls the band `To-do`, the plan calls it `BACKLOG`, and only the board is the plan's surface. The phone queue-view's filter chip keeps `To-do`. The **glyph** is still derived from `STATUSES` and never retyped; only the display word is local, and the key is matched against the vocabulary's own `key` so it cannot silently stop matching.

⭐ **Every listed row carries a derived plain-English line — or an honest admission that it cannot.** Banding fixed _how much_ of the board you read; it did nothing about the rows themselves, which were an ID plus the item's own heading. Those headings are emphasis-laden shorthand written for a reader who already has the context, so the board was legible only to someone who had memorised the identifiers. A board you cannot read is a board you cannot pick the next task off, which is the one job it has. Each row now gains a sub-line in ordinary prose, extracted from the item body: its completion condition, the answer-shaped field decision items carry instead, or an explicit self-description — first hit wins.

⛔ **The summary is EXTRACTED or the row is MARKED. There is deliberately no third branch.** An `id → description` table maintained in the generator would be a second copy of prose whose original lives in the source document, and this repo has more than once shipped a hand-copied constant that silently fell out of step with the thing it duplicated. Composing a plausible description for an item that does not contain one is worse still: a confident wrong summary is indistinguishable from a real one and gets believed, whereas a printed gap is visible and becomes a worklist. So the board also reports, per band, how many rows are summarised versus marked — two numbers, never summed into a single coverage percentage that would flatter the surface it measures, the same rule the closed-item block follows.

⚠ **The field label must open its own line, and that anchor is load-bearing rather than tidy.** An unanchored search matches the same words used mid-sentence: measured on the live corpus, an ordinary prose aside that merely _ended_ in one of the field names, with a choice list directly beneath it, was harvested as though it were the field, and the row then carried an answer the item had never given — the fabrication mode above, arrived at by accident. Suite 248.9c locks it, and the lock had to be **re-broken to be believed**: the first fixture put a blank line between the aside and the list beneath it, which made the harvested paragraph empty and let the length floor reject it, so the test passed against a deliberately broken extractor. A fixture that does not reproduce the defect proves only that the code runs. Decorative glyphs are stripped by **Unicode property** rather than a listed set, for the same anti-drift reason: the parser's exported vocabulary covers status glyphs and one emphasis marker, the source uses several more, and a newly-adopted one would otherwise start leaking into the board with nothing to notice.

⛔ **WHICH clause a row quotes is a separate correctness problem from WHETHER it gets one, and it is the more dangerous of the two.** A marked row admits it says nothing; a badly-summarised row asserts something false in exactly the same voice as a correct one. Two such defects were found by a reader looking at the real board, not by any check. **(1) First-match-wins does not survive sub-items** — a long item can contain a nested sub-item stating its own completion condition, and document order then describes the whole item by a detail buried inside it. Top-level now outranks document order. Indentation was **verified as the discriminator before being trusted**: across the live corpus these labels are bimodal at column 0 rather than graded, and no item carries only indented labels, so preferring column 0 cannot strand a row — and the fallback to document order is kept structural rather than statistical, so an item that breaks that measurement loses nothing. **(2) Struck-through text was being harvested.** The source strikes a clause when it is _discharged_, so the board rendered crossed-out markup exactly where the remaining work belonged; a reader could reasonably conclude nothing was left on the item.

⚠ **Discharged work is removed at CLAUSE granularity, because the annotation trailing a strike belongs to it.** Removing only the struck span leaves orphaned credit for finished work ("— done at ⟨ref⟩") sitting where the outstanding work should be — still misleading, merely no longer visibly crossed out. Dropping the whole clause surfaces what is still owed. Where clause-dropping would empty the field — an item that strikes each clause and then states the position in prose _inside_ the last struck one — span-removal is tried before giving up, which keeps the sentence saying work remains. ⛔ **If nothing readable survives, the row is MARKED and the remaining field kinds are not tried.** That is a stated choice, not a fallthrough: the item did state this field and it has been discharged, so reaching past it to a different field would describe the item by something it never offered as its summary. The struck spans are also dropped in the shared cleaner, so a future harvesting path cannot reintroduce the leak by forgetting.

⚠ **A later clause that AMENDS an earlier one is read from the author's word, never inferred.** Both relationships occur in the source and look identical structurally while meaning opposite things: a clause marked as _superseding_ replaces the earlier text, while one marked as _added_ extends it and reads as a non-sequitur alone. Measured against the real corpus, "prefer the last" repairs the superseded case and **breaks** the additive ones, so neither pure ordering rule is correct — only an explicit supersession marker is honoured. Where a clause carries a parenthetical **scope**, that scope is surfaced in the row's label rather than left as residue at the front of the prose: an item may state several conditions scoped to different parts of itself, and an unlabelled one reads as though it were the whole item's. ⚠ **What this deliberately does not do is choose between several equally top-level clauses on meaning** — an item carrying more than one gets its first, scope-labelled. Picking "the real one" would require interpreting the author's qualifiers, which is the fabrication risk this surface exists to refuse; the fix for such an item is authoring, not extraction.
...
⚠ **That makes a false GREEN in `--check` uniquely expensive, and it shipped with two** (found by reading the code, fixed 2026-08-13). It silences not one problem but the entire fail-closed design, because nothing else on this path is ever allowed to go red. (1) An **absent artifact** printed "nothing to verify" and exited 0 — a green-that-skipped living inside the guard whose whole job is to catch greens that skipped, which made _deleting_ `ROADMAP.md` a way to make its own check pass. (2) A **stale artifact** passed: the board records the sha256 of the `QUEUE.md` it was built from, and nothing ever compared that to the current source, so a board generated from a queue weeks out of date read exactly like a fresh one — the same "stale reads as current" failure the BLIND renderer already refuses to commit _inside_ the document, arriving by the back door.

`--check` now fails on four conditions, ordered so the strongest signal wins: **missing → blind → unverifiable → stale.** Blindness is tested before freshness deliberately, because a blind board built from the current queue has a perfectly matching fingerprint. A board carrying no readable fingerprint fails too: unverifiable is not a pass, and treating a missing fingerprint as "probably fine" would convert the check into a permanent green the moment the provenance shape changed.

⛔ **The skip is on the TREE, never on the artifact.** No planning tree at all (a public clone, by design under F04) still exits 0 — an absent _tree_ and an absent _board_ are different facts and only the second is a failure. The one emitter/one parser rule applies to the provenance block for the same reason `ITEM_ID_RE` has exactly one copy: `--check` parses the fingerprint the renderers write, so a drifting second copy would silently degrade freshness-checking into a permanent pass.

⚠ **What it still does not prove, stated rather than implied:** the board matches the **source**, not the **generator**. A board produced by an older version of the script from an unchanged `QUEUE.md` passes. The full regenerate-and-byte-compare of Protocols 47/52/53 is unavailable here because this artifact stamps the app repo's git `HEAD`, which changes on every unrelated commit — comparing it would red the next push after any commit at all (the 247.10 trap, locked by Suite 248.7h). Regenerating in the same commit as any rendering change is the discipline that covers the gap.
