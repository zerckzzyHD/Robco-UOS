...
├── tests/
│   ├── robco-diagnostics.ps1   28KB    2856-test pre-commit audit
│   ├── robco-diagnostics.js    36KB    2856-test Node runner (parity with .ps1)
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (only if a served file is staged; skipped for doc/CI/test-only commits), then the 2856-test persistence audit
