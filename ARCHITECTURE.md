...
├── tests/
│   ├── check-persistence.ps1   28KB    355-test pre-commit audit
│   ├── check-persistence.js    36KB    355-test Node runner (parity with .ps1)
...
The pre-commit hook enforces this rule automatically. Before running the 355-test suite, it reads `CACHE_NAME` from the staged `sw.js` and compares it to `origin/main:sw.js`. If they match, the commit is blocked with a clear error. A missed bump is impossible to commit past.
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (fails immediately if not bumped), then the 355-test persistence audit
