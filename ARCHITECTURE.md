...
├── tests/
│   ├── check-persistence.ps1   28KB    386-test pre-commit audit
│   ├── check-persistence.js    36KB    386-test Node runner (parity with .ps1)
...
The pre-commit hook enforces this rule automatically. Before running the 386-test suite, it parses the staged `CACHE_NAME` against `origin/main:sw.js` and requires a strict monotonic increase in the `-rN` revision number when `APP_VERSION` is unchanged — equal or lower revs are blocked. When `APP_VERSION` changes, the revision can reset. A missed or decremented bump is impossible to commit past.
...
- [ ] `git commit` — pre-commit hook runs the CACHE_NAME guard first (fails immediately if not bumped or decremented), then the 386-test persistence audit
