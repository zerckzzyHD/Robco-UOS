...
│   ├── roadmap-generate.js    Bands QUEUE.md's items into a generated ROADMAP.md in the private tree (`npm run roadmap`; `--check` via `npm run roadmap:check` exits 1 on a BLIND board). Reuses queue-view.js's parser + vocabulary (never a second parser); a structural parse gap renders the WHOLE doc BLIND rather than a partial list, and the generator itself always exits 0 — it is a reporter, never a blocker
