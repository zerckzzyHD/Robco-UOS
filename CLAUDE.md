...
| `ROADMAP.md`      | ⛔ **Nothing — never edit it by hand.** GENERATED-class (`rules/docs-and-library.md`): produced from `QUEUE.md` by `npm run roadmap` into the **private** planning tree, alongside the queue itself (F04 — it is derived from private material and must never land in this public repo). Regenerate it rather than correcting it; if it reads wrong, the fault is in `QUEUE.md` or the generator. It renders **BLIND** rather than partial when the parse is not trustworthy, and `npm run roadmap:check` exits 1 when the board is **missing, blind, unverifiable, or stale** against `QUEUE.md`'s sha256 (⚠ the first and last were false greens until 2026-08-13 — an absent board printed "nothing to verify" and passed, and a board built from an older queue read exactly like a fresh one). A skip is on the planning TREE only; an absent board on a machine that has the tree is a failure. |
...
**Effort tier — the second dial, set per session (owner-approved 2026-07-31).** Alongside the model, Dispatch sets a spawned session's **reasoning-effort tier** per session, at will, exactly as it switches model per stage. **The ladder: Low / Medium / High (the default) / Extra / Max / Ultracode** — reserve Max/Ultracode for gnarly, high-stakes work where a wrong call is expensive; High is right for routine work. **The mechanism is the TWO-MESSAGE pattern:** send a message that is **exactly `/effort <level>` and nothing else** (it parses as a slash command, runs 0 turns, and the session idles), **then** send the task as a **separate** follow-up, which runs at that tier. ⛔ **Inlining `/effort` with a task is FORBIDDEN — the slash command swallows the whole task as its argument and nothing runs at all**, a silent no-op rather than an error. **The pre-build PLAN decides the tier per spawned session** (it is a budget decision, so it belongs in the plan, not improvised at launch), and **Dispatch ANNOUNCES the tier on every change**, in the same breath as a model switch — a silent tier change makes the effort budget unauditable. Full write-up, including what is VERIFIED versus merely CLAIMED about what a tier does: `planning/control-plane/EFFORT_CONTROL_SPIKE.md`.
...
**Capability — a host session CAN run it (verified 2026-07-18).** From the **Bash tool** (shell user `kadyn`), `node`/`powershell.exe` see the memory folder (`AppData\Roaming\Claude\local-agent-mode-sessions\…\agent\memory`), the archive working copy, and `library/`/`planning/` — so `sync.ps1` runs successfully and the backup is effectively **automatic**, with the nudge as its trigger. The **one caveat**: run it from the **Bash tool only**, never the **PowerShell tool** — the PowerShell tool's sandbox (a different local user context) cannot see `AppData\Roaming\Claude`, so memory discovery there finds nothing and the sync fails loudly. This is the "one shell can see memory, another can't" quirk: the Bash-launched shell is the one that can. If a future host/shell genuinely cannot reach the archive or the memory store, that is the case where the session reports upward instead (per the obligation above).

**Automated nudge — a fail-safe AGENT trigger, never a blocker.** The pre-push hook runs `node scripts/backup-nudge.js` **after** the gate, suffixed with `|| true`. It is a fail-safe reminder, never a gate (Protocol 33 DNA): it can never fail or block a push (it always exits 0, and `|| true` is a second guarantee).

⛔⛤ **THE PREDICATE IS CONTENT EQUIVALENCE, AND IT USED TO BE A CLOCK (rebuilt 2026-09-01, item `GV14`).** It compared local file **mtimes** against **the archive repo’s last commit timestamp** — so **any** commit to the archive discharged it, whether or not that commit backed anything up. ⚠ No bug and no race were required: two sessions and a clock are sufficient, which is this project’s ordinary operating state. **Measured**: three artifacts were unbacked while it reported nothing owed — `library/TEST_CATALOG.md`, the orchestrator’s `MEMORY.md`, and a memory file created that same session expressly to preserve an owner ruling before it was cut from elsewhere. ⛔⛔ **And "check presence" would have been just as silent** — `TEST_CATALOG.md` was **present and 144 bytes stale**. ⭐ *"The file is there"* and *"the file is current"* are different questions, and a backup answers the second. It now compares every watched artifact **byte-for-byte** against the archive tree at a **pinned sha**.

⭐⭐ **AND IT IS NEVER SILENT — silence was the entire failure.** *An instrument that cannot tell "I checked and it is fine" from "I could not check" is indistinguishable from one that is broken.* Every path prints exactly one verdict: **BACKED UP** (naming how many artifacts were compared), **NOT BACKED UP** (naming what differs and what is absent, with the exact Bash-tool sync command, addressed to the session rather than the owner), **UNOBSERVABLE** (it tried and could not establish the answer — e.g. the archive is present but unreadable), or **NOT CONFIGURED** (there is no archive on this machine). ⚠ **The last two are different facts and are not collapsed**: a clone without the private archive is a normal by-design state for this PUBLIC repo, while an unreadable archive is a failure to measure — and neither may read as *"your work is safe"*. ⛔ Archive access is read-only and **index-free** (`rev-parse` + `ls-tree` at a pinned sha, all `--no-optional-locks`): that repo has a live writer, and `git status` alone takes the index lock and can strand a lock file indistinguishable from a crashed writer. The nudge is a backstop trigger only — the hook must stay fast and side-effect-free, so it never runs the sync itself; the **session** does, per the obligation above. Guarded by Suite 237.
