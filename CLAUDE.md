...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (3453 tests via gate:fast)
...
- **3453 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 3453 tests must pass
...
- [ ] Lint, format, commit (3453 tests) → Protocol 2
...
The definitive verification step is `tests/render-check.mjs` — a Playwright render-check that loads the page at 360px and 412px and asserts no horizontal overflow and no focus-zoom. Run it outside the 3453-test pre-commit gate whenever map or mobile layout changes land. It is the only check that catches real pixel/overflow regressions.
...
- The **full pre-commit / pre-push gate** runs and must pass exactly as on `main`: ESLint with **zero** errors/warnings, Prettier clean, the canonical Node test runner (`tests/robco-diagnostics.js`) green (238 suites, 3453 tests), plus the push-boundary browser checks — boot-smoke, render-check, the a11y baseline-diff, and the `tests/test.html` runtime audit.
