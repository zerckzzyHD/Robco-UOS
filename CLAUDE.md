...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (1156 tests via gate:fast)
git push origin main  # CACHE_NAME must already be bumped (Protocol 1)
```

- **1156 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 1156 tests must pass
...
- [ ] Lint, format, commit (1156 tests) → Protocol 2
...
- The boot manifest file-selection ternary in `index.html` (pending WU-A5 refactor)
