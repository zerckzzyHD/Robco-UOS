...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (3276 tests via gate:fast)
...
- [ ] Run `git commit` — 3276 tests must pass
...
- [ ] Lint, format, commit (3276 tests) → Protocol 2
...
The definitive verification step is `tests/render-check.mjs` — a Playwright render-check that loads the page at 360px and 412px and asserts no horizontal overflow and no focus-zoom. Run it outside the 3276-test pre-commit gate whenever map or mobile layout changes land. It is the only check that catches real pixel/overflow regressions.
