...
- **ARCHIVE** — frozen point-in-time snapshots: the audits, plans, mockups, and slates under `planning/`. These carry a "snapshot as of DATE — not current truth" framing and are never updated to track current code; their reasoning is reused as historical input, never trusted as current fact. The pre-2.8.0 audits are filed under the version they actually audited (their internal baseline stamp), not under 2.8.0: the six v2.6.0-baselined ones (`planning/2.6.0/audits/CODE_QUALITY_AUDIT.md`, `planning/2.6.0/audits/PERFORMANCE_AUDIT.md`, `planning/2.6.0/audits/ACCESSIBILITY_AUDIT.md`, `planning/2.6.0/audits/TEST_STRENGTH_AUDIT.md`, `planning/2.6.0/audits/UI_CONSISTENCY_AUDIT.md`, `planning/2.6.0/audits/CLOUD_AUDIT.md`) and the two v2.7.0-baselined ones (`planning/2.7.0/audits/TOKEN_USAGE_AUDIT.md`, `planning/2.7.0/audits/FILE_AUDIT.md`) are ARCHIVE-class — they audited a codebase roughly 30% smaller than today's; do not trust their measurements or reuse their proposals without re-verifying against current code.
...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (3411 tests via gate:fast)
...
- **3411 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 3411 tests must pass
...
- [ ] Lint, format, commit (3411 tests) → Protocol 2
...
The definitive verification step is `tests/render-check.mjs` — a Playwright render-check that loads the page at 360px and 412px and asserts no horizontal overflow and no focus-zoom. Run it outside the 3411-test pre-commit gate whenever map or mobile layout changes land. It is the only check that catches real pixel/overflow regressions.
...
- The **full pre-commit / pre-push gate** runs and must pass exactly as on `main`: ESLint with **zero** errors/warnings, Prettier clean, the canonical Node test runner (`tests/robco-diagnostics.js`) green (237 suites, 3411 tests), plus the push-boundary browser checks — boot-smoke, render-check, the a11y baseline-diff, and the `tests/test.html` runtime audit.
