...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (1078 tests via gate:fast)
git push origin main  # CACHE_NAME must already be bumped (Protocol 1)
```

- **1078 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 1078 tests must pass
...
- [ ] Lint, format, commit (1078 tests) → Protocol 2
