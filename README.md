...
| **Testing**     | PowerShell persistence audit  | 831-test pre-commit gate                  |
...
├── tests/
│   ├── check-persistence.ps1  Pre-commit 831-test persistence audit (PowerShell)
│   └── check-persistence.js   Pre-commit 831-test persistence audit (Node)
...
git add -A
git commit          ← cache-bump guard runs first, then persistence audit (831 tests)
