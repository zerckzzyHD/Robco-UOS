...
| **Testing**     | Node + Playwright                                | 3105-test Node gate + boot-smoke / render / a11y checks                     |
...
├── tests/
│   ├── robco-diagnostics.js   Node persistence/structure audit (3105 tests, 224 suites — the single canonical runner)
...
All unreleased work goes to **`dev`**; **`main` is release-only**. Each commit keeps docs + the 3105-test count in sync and bumps `CACHE_NAME` when a served file changes.
...
- **Fallout 3 landscape Pip-Boy** _(early, in progress)_ — rotate a Fallout 3 campaign sideways and the same three subsystem keys, the UPLINK/CHASSIS/SETTINGS controls, and a flat directory recast as a real Pip-Boy 3000 casing: three domed lamps, a radio knob, a status gauge, and a toggle switch. Each screen's sub-sections get their own row of tabs on the glass and remember which one you last had open. Portrait keeps today's layout untouched, and New Vegas is unaffected either way; a fully dressed casing and the Vault Boy status view are still to come
...
- **A self-improving gate** — **3105 tests across 224 suites** in the canonical Node runner, plus Playwright boot-smoke / render-check / a11y baseline and a `test.html` runtime audit; CI + a nightly run back it up
