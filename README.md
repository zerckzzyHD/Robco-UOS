...
| **Testing**     | PowerShell persistence audit  | 402-test pre-commit gate                  |
...
├── tests/
│   └── check-persistence.ps1  Pre-commit 402-test persistence audit
...
git add -A
git commit          ← cache-bump guard runs first, then persistence audit (402 tests)
...
- 402-test automated persistence audit (DOM binding, Protocol 4 enforcement, migrateState mock execution, reputation 2D matrix, CRUD function existence, CAMPG tab DOM binding, campaignMode + playthroughType Protocol 4, render contracts, CSS invariants, SW invariants, structural integrity, detail-current dedup guard, gate guards for critical UI controls, prohibited patterns, protocol completeness, AI contract lock, architectural boundaries, assets completeness, and CSS hygiene guards)
