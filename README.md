...
| **Testing**     | Node + Playwright                                | 3392-test Node gate + boot-smoke / render / a11y checks                     |
...
├── tests/
│   ├── robco-diagnostics.js   Node persistence/structure audit (3392 tests, 236 suites — the single canonical runner)
...
All unreleased work goes to **`dev`**; **`main` is release-only**. Each commit keeps docs + the 3392-test count in sync and bumps `CACHE_NAME` when a served file changes.
...
- **A self-improving gate** — **3392 tests across 236 suites** in the canonical Node runner, plus Playwright boot-smoke / render-check / a11y baseline and a `test.html` runtime audit; CI + a nightly run back it up
