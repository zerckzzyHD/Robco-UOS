...
| **Testing**     | PowerShell persistence audit  | 1268-test pre-commit gate                 |
...
├── tests/
│   ├── check-persistence.ps1  Pre-commit 1268-test persistence audit (PowerShell)
│   └── check-persistence.js   Pre-commit 1268-test persistence audit (Node)
...
git add -A
git commit          ← cache-bump guard runs first, then persistence audit (1268 tests)
