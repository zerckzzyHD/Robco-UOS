...
| **Testing**     | PowerShell persistence audit  | 253-test pre-commit gate                  |
...
├── tests/
│   └── check-persistence.ps1  Pre-commit 253-test persistence audit
...
git add -A
git commit          ← cache-bump guard runs first, then persistence audit (253 tests)
...
- 253-test automated persistence audit (DOM binding, Protocol 4 enforcement, migrateState mock execution, reputation 2D matrix, CRUD function existence, CAMPG tab DOM binding, campaignMode + playthroughType Protocol 4, render contracts, CSS invariants, SW invariants, structural integrity, detail-current dedup guard)
