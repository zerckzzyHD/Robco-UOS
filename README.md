...
| **Testing**     | Node + PowerShell + Playwright                   | 1950-test gate at parity + boot-smoke / render / a11y checks |
...
├── tests/
│   ├── robco-diagnostics.js   Node persistence/structure audit (1950 tests, 159 suites)
...
All unreleased work goes to **`dev`**; **`main` is release-only**. Each commit keeps docs + the 1950-test count in sync and bumps `CACHE_NAME` when a served file changes.
...
- **A self-improving gate** — **1950 tests across 159 suites**, mirrored in the Node and PowerShell runners at exact parity (per-suite composition, not just the grand total), plus Playwright boot-smoke / render-check / a11y baseline and a `test.html` runtime audit; CI + a nightly run back it up
