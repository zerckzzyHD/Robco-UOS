...
| **Testing**     | Node + Playwright                                | 3272-test Node gate + boot-smoke / render / a11y checks                     |
...
├── tests/
│   ├── robco-diagnostics.js   Node persistence/structure audit (3272 tests, 232 suites — the single canonical runner)
...
All unreleased work goes to **`dev`**; **`main` is release-only**. Each commit keeps docs + the 3272-test count in sync and bumps `CACHE_NAME` when a served file changes.
...
- **A self-improving gate** — **3272 tests across 232 suites** in the canonical Node runner, plus Playwright boot-smoke / render-check / a11y baseline and a `test.html` runtime audit; CI + a nightly run back it up
