...
| **Testing**     | PowerShell persistence audit  | 1531-test pre-commit gate                 |
...
├── tests/
│   ├── robco-diagnostics.ps1  Pre-commit 1531-test persistence audit (PowerShell)
│   └── robco-diagnostics.js   Pre-commit 1531-test persistence audit (Node)
...
git add -A
git commit          ← cache-bump guard runs first, then persistence audit (1531 tests)
