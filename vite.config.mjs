...
const VIEW_CHAIN = [
  ...
  // ⚠ A DATA READER, so it belongs on this list by the rule at the top — even
  // though it is read ONCE at startup rather than per request. Listing it keeps
  // the dependency closure complete (Suite 260.12 checks the closure, not the
  // filenames), and an edit to the resolver during a long-running session is
  // then visible on the next config evaluation rather than a restart later.
  './scripts/dev-host.js', // the tailnet hostname, resolved off tracked source
];
...
// ── ⛔ THE TAILNET HOSTNAME, RESOLVED AT STARTUP AND NEVER DEFAULTED ──────────
//
// Resolved HERE, at module scope, so the refusal happens before the server binds
// anything: a dev server that came up on the wrong host list does not fail, it
// serves a "Blocked request" page to the phone, which reads like a network fault.
//
// ⚠ THE REFUSAL IS SCOPED TO `serve`, AND THE SCOPE COMES FROM VITE, NOT FROM
// ARGV. This config is also loaded by tooling and by the gate, and a public clone
// has no `dev-host.local.json` by design — so throwing unconditionally would break
// a checkout that never intended to run a dev server. `allowedHosts` is meaningless
// outside `serve`, so outside it the absence is carried as an empty list.
//
// ⛔⛤ THE FIRST VERSION OF THIS SNIFFED `process.argv` FOR 'serve'/'dev', AND ITS
// OWN RED CONTROL CAUGHT IT: `vite --port 5199` carries NEITHER token (bare `vite`
// IS serve — 'serve' is the default command and is usually omitted), so the guard
// read that as "not serving" and the server came up with an EMPTY host list. A
// refusal that does not fire is worse than none, because the page it produces is
// the same "Blocked request" the guard exists to prevent. `defineConfig` receives
// the real command from Vite; that is the only thing that actually knows.
const DEV_HOST_R = freshRequire('./scripts/dev-host.js').resolveDevHosts();
const DEV_HOSTS = DEV_HOST_R.ok ? DEV_HOST_R.hosts : [];
function requireDevHostsFor(command) {
  if (command !== 'serve' || DEV_HOST_R.ok) return;
  // ⛔ Printed and then thrown: Vite prints a config error as a stack, and a stack
  // is not the message somebody whose phone stopped working needs to read.
  console.error(freshRequire('./scripts/dev-host.js').refusalMessage(DEV_HOST_R.why));
  // ⚠ The prefix deliberately avoids the literal `host:` followed by a quote —
  // Suite 249.6 scans this file for `host:\s*'…'` to prove the bind address is a
  // loopback, and its first-match-wins regex read THIS string as that setting.
  // Caught by 249.6 going red on a correct config; renaming the prefix is the
  // honest fix, since the collision was introduced here rather than there.
  throw new Error('tailnet hostname not configured — ' + DEV_HOST_R.why);
}
...
export default defineConfig(({ command }) => {
  // ⛔ Before anything binds. Vite hands us the real command here — see
  // requireDevHostsFor's header for the argv-sniffing version this replaced.
  requireDevHostsFor(command);
  return {
    ...
    server: {
      ...
      //
      // ⛔⛤ THE HOSTNAME IS NOT IN THIS FILE, AND THAT IS THE POINT (owner ruling
      // 2026-09-08, option (a)). It used to be — a literal `allowedHosts` entry naming
      // this machine, committed to a PUBLIC repository on 2026-08-24 (`838c839`) and
      // public every day since. It is resolved at startup instead, from a gitignored
      // `dev-host.local.json` (or `ROBCO_DEV_HOST`); see scripts/dev-host.js for the
      // resolution order and why the FILE is the primary rather than the env var.
      //
      // THE VALUE IS TAILNET-ONLY. A `*.ts.net` MagicDNS name resolves only inside a
      // private tailnet; it is not a public domain and does not resolve on the open
      // internet. That is why this is hygiene rather than an incident — and it is
      // also why the fix is worth making anyway: a machine identifier has no reason
      // to be world-readable, and moving it out closes the class rather than this
      // instance. Do NOT copy this pattern for a public hostname.
      //
      ...
      // trusted. ⭐ The resolver refuses a wildcard for the same reason, so an
      // untracked file cannot quietly re-make that decision where nobody reviews it.
      //
      // Example of what the untracked file holds — a PLACEHOLDER, never a real host:
      //   dev-host.local.json  { "allowedHosts": ["<your-machine>.tailnet.ts.net"] }
      allowedHosts: DEV_HOSTS,
