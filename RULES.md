...
git add -A
git commit          # Pre-commit hook: 137 tests must pass
git push origin main
```

- **137 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 137 tests must pass
...
- [ ] Lint, format, commit (137 tests) → Protocol 2
...
**Test suite:** 137 tests across 9 suites in `tests/check-persistence.ps1` (PowerShell) and `tests/check-persistence.js` (Node). Covers parser sanity, autoImportState coverage, faction registry, skill keys, save envelope, file upload, cloud sync, backward compatibility, and registry structural integrity.
