...
git add -A
git commit          # Pre-commit hook: cache-bump guard runs first, then fast gate (1531 tests via gate:fast)
git push origin main  # CACHE_NAME must already be bumped (Protocol 1)
```

- **1531 tests must pass.** If fewer pass, something is broken. Investigate before committing.
...
- [ ] Run `git commit` — 1531 tests must pass
...
- [ ] Lint, format, commit (1531 tests) → Protocol 2
...
- The **full pre-commit / pre-push gate** runs and must pass exactly as on `main`: ESLint with **zero** errors/warnings, Prettier clean, **both** test runners (`tests/robco-diagnostics.js` + `tests/robco-diagnostics.ps1`) green and **at parity** (identical suites, per-suite counts, and 1531 total), plus the push-boundary browser checks — boot-smoke, render-check, the a11y baseline-diff, and the `tests/test.html` runtime audit.
