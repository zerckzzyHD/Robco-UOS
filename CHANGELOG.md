...
- Added a gentle reminder to back up the development notes and design files that only ever exist on the developer's own machine. Those files aren't part of the public project, so if the machine were lost they'd be gone for good; a small private backup copy exists for exactly that reason. Now, whenever those files have changed since the last backup, a short reminder prints just before new work is published, nudging a fresh backup. It's only ever a reminder — it can never delay, block, or interfere with publishing, and on any machine that doesn't have the private backup it stays completely silent and invisible. Nothing you can see changed.
