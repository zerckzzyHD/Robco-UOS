## [Unreleased]<!-- Tests: 3270/3270 | Cache: robco-terminal-v2.8.0-r39 -->
...
- Finished the New Vegas armor check by verifying the last leftover items — the hats, helmets and outfits whose stats live on a separate wiki page. Their weights and values are now confirmed, so 99 of the 103 armor entries are fully checked. The four still left unconfirmed are a Fallout 3 outfit that isn't in New Vegas, a "Vault Utility Suit" that has no matching New Vegas item, and two duplicate names for the NCR Ranger combat armor (which is already in the list correctly) — all left untouched rather than guessed. Also fixed the Chinese Stealth Armor's description, which still claimed it turns you invisible like the Fallout 3 version; the New Vegas one only gives a +5 Sneak bonus, so that's what it now says.
