## [Unreleased]<!-- Tests: 1800/1800 | Cache: robco-terminal-v2.7.0-r28 -->
...
- Added a hidden developer tool that only ever appears on the private staging build used for testing — it is never present on the live site players use. It shows what the terminal's new "living machine" engine is doing in real time and lets a developer manually step it through its states while testing new atmospheric features as they're built. It is purely a testing aid: it never touches your campaign, save, or stats, and a player on the real site will never see any trace of it.
