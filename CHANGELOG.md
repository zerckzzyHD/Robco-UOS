...
- Gave the project's reusable "quick-start" instruction pack — a short summary a work session reads before it even opens the project — a proper home inside the project itself, and an automated check that keeps it honest. It had drifted badly (it named a folder that no longer exists, a testing tool that was deleted, and an out-of-date rule), because it lived only as an installed copy nobody could review or correct. Now the real version is kept with the project's own files, and the build fails if it ever again names a location, rule, or file that no longer exists. A gentle, never-blocking reminder also flags when the installed copy has fallen behind the corrected one so it can be refreshed. Nothing you can see changed.
