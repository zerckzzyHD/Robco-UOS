## [v2.5.0] — Unreleased<!-- Tests: 572/572 | Cache: robco-terminal-v2.0.1-r59 -->
...
- Added save integrity protection across all save and load paths (Suite 51, 572 total across 55 suites). Every save slot, exported file, and cloud save is now stamped with a checksum and a schema version. When you load any save, the app verifies the checksum — if it doesn't match, you get a clear warning and can choose whether to load anyway. If a save file was made on a newer version of the app than the one you're running, the app warns you before loading it. Old saves without a checksum load normally with no change to existing behavior. A rolling backup ring of three automatic snapshots is taken before any load operation so you can always undo an accidental load. The RESTORE BACKUP button (next to IMPORT SAVE) lists all available backups and lets you pick one to restore — confirm-gated, fully sanitized, same guarantees as any other load. Quota failures on backup writes degrade gracefully with no crash. The duplicate checksum helper in cloud.js was removed and replaced with the shared helper from state.js.
