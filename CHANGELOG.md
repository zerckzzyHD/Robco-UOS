## [Unreleased]<!-- Cache: robco-terminal-v2.8.5-r58 -->
...
- The queue page's "needs you" number was merging two different errands. Some items ask you a question — those are a sitting you book. Others need your hands: be somewhere, run something, authorise something whose evidence is already gathered. Those are a task list. Reading them as one number is wrong by about half even when the total is right, because you cannot act on the answer. The page now shows them as two, taken from the classification the planning tree already records for every item, and each one opens into the actual list so you can check it rather than trust it. Every row shows how it was worked out — read in full, or skimmed from the heading — because this board records decisions inside item bodies without updating the headings, so a skimmed row may already be answered; the rows that were read in full are listed first. The page says out loud that the count is an upper bound for that reason. It also no longer lets a zero mislead: the older, hand-kept decision list has been empty since September 3rd, and "0" under a label about decisions reads as "nothing is waiting on you" — it now says a zero there means no declared row is open, and nothing more.
