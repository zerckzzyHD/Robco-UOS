## [Unreleased]<!-- Cache: robco-terminal-v2.8.5-r68 -->
...
- The developer's private queue page now shows the two things the build list was missing: WHEN a piece of work matters, and WHICH project it belongs to. The list had grown to hold six projects and every timescale at once — an idea for months away sat in the same column, and counted the same, as something blocking work today. Each item can now say whether it blocks work now, comes next, or is a someday-if thought with a condition attached, and the page reads that from the item itself. A someday-if item is in no total anywhere on the page, which is the whole point of having the field: if it still counted, nothing would have changed. Where leaving one out makes a number smaller, the page says so and names the item, rather than quietly showing you a smaller figure. Almost no item carries the new field yet — it is being filled in now — so the page shows those as "unset" and never guesses: guessing "blocks work now" would inflate what looks urgent, and guessing "someday" would hide real work. And if it cannot read the field at all it says that instead, because "nobody has set this yet" and "I could not look" are different facts that would otherwise print the same big number. The per-project split comes from the count the planning tree already keeps, and the page states plainly what that count does not yet separate: the harness is folded in with the control plane, and Binder has no column of its own. Both fields are now filled in across the whole board, and the page reads them from where the board actually keeps them: fifty-five of the four hundred and eleven items are someday-if thoughts, which is one in eight of the list that should never have been counted as work. Taking them out drops the backlog from 251 to 221, the parked pile from 25 to 3, and twenty-nine items off the list of things waiting on you. Fourteen items nobody could place are shown as unknown rather than guessed into a column, and where an item and the board-wide pass disagree about it the page names both answers instead of quietly picking one. The board can now be filtered to one project at a time — the harness and Binder are their own columns for the first time — On the project column the page now refuses to guess: where the only reason for a label was a keyword match — which the tool that made them measured as wrong about one row in three — the item reads unknown instead. That is 192 of the 411, and it will look like the tool being useless; it is the opposite, it is the tool declining to invent 180 answers, and the page says so in those words. A count can be read as an estimate, but a label sitting next to an item gets read as a fact about that item and someone acts on it.

### Fixed

- The tailnet address of the developer's own machine had been sitting in the public repository since 24 August, written into the development server's configuration as the one hostname it would answer to. It is not a security hole in the usual sense — that address only resolves inside his own private network and is never reachable from the internet, so knowing it grants nobody anything — but the name of somebody's machine has no reason to be world-readable, and it was found only because three unfinished branches carrying it in a test receipt were being cleaned off the public remote. The value now lives in a small file on the machine itself that is never committed; the tracked configuration carries a labelled placeholder instead, and the development server refuses to start at all when the real value is missing, with a message naming exactly which file to create and how to look the name up. It deliberately does not fall back to a guess: a wrong entry there does not announce itself, it just makes the phone show a "blocked request" page that reads like a broken network. The same treatment was applied everywhere else the name appeared, and an automated check now scans every tracked file for anything shaped like such an address, so a different machine's name cannot quietly take its place. The name stays in the repository's past history, deliberately and on the record — rewriting shared history would break every existing copy of the project for no benefit this change claims.
...
- The queue page's "needs you" number was merging two different errands, and then quietly telling you nothing was waiting. Some items ask you a question — those are a sitting you book. Others need your hands: be somewhere, run something, authorise something whose evidence is already gathered. Those are a task list. Reading them as one number is wrong by about half even when the total is right, because you cannot act on the answer. The page now shows them as two. Each counts only the rows somebody has actually read all the way through, and says "at least this many" rather than pretending to be exact; the rows that were merely skimmed from a heading are not folded in, because this board records decisions inside item bodies without updating the headings, so a skimmed row may have been answered weeks ago. Those skimmed rows get a tile of their own that says plainly it cannot tell either way — on the live board that is eighty-five of them, and folding them in was what made the old number roughly three times too big. Both lists open so you can check them rather than trust them, split at exactly the line the count is drawn at. And the older, hand-kept decision list — which has been empty since September 3rd, so the page had been showing "0" under a label about decisions, which reads as "nothing is waiting on you" — now says it is empty and measured nothing, rather than showing a zero. A list that still has rows on it and finds none of them open keeps showing its real zero; the difference is between a count and a missing source.
- Tapping a project on the queue page did not change the numbers. The band rows kept their headline figure and only a small caption underneath responded, so the biggest, most readable number on each row was the one that did not answer the question just asked — and it sat next to a caption counting a different set again. Neither was stale: one counted every item the board files in that band with the someday-if ones taken out, the other counted the rows actually drawn on screen with them left in, which is why they agreed on three bands and differed on the other three. Each band number now follows the filter, the Backlog included, and the caption is gone rather than corrected — two numbers of different sets on one row was the fault, so leaving a second one would not have been a fix. The counts are worked out where the page is built, never in the browser, so filtering can never produce a second answer; a band with nothing in it says nought rather than a question mark; and each project button now shows exactly the total its own bands add up to. The page also states, once, that it carries two denominators on purpose — the whole queue, and the queue minus the someday-if items — and what the difference between them is made of.
- Numbers that sit next to each other on the queue page now have to be about the same thing, or the page has to say they are not. Every defect on that page in one evening was the same shape: two figures, each correct and each already saying where it came from, put side by side as though you could subtract one from the other. Saying where a number comes from turned out not to be enough — two numbers can both name their source honestly and still be counting different populations. Each figure now also records which set it counts over, and a check refuses any group of figures that mixes them silently; where a group genuinely answers several questions at once, as the top row of the page does, it now says so in a sentence rather than leaving you to notice. The check also fails when it finds nothing to check, because its own first version quietly matched nothing and reported a clean result. And the project filter is now exercised by an automated browser run on every full check — clicking each project and comparing the numbers on screen against the ones the page published — because reading the data a script is given is not the same as watching the script work.
- The build board now says out loud that its own band counts leave something out. Its numbers include the someday-if items, and the queue page takes them back out again, which looks like one document quietly correcting another. It is not: the board is built to be a pure function of the queue file alone, and the someday-if labels live in a different file, so of the fifty-nine such items the board can see none at all. Teaching it to read that second file would have made it depend on two inputs while its freshness stamp only covers one — meaning the check that says the board is current would have kept saying so whenever only the other file moved. So the board states the limit instead, with the count it can honestly give, and the page no longer phrases its own adjustment as a correction of a mistake. The real repair is one step further back and is not in this project: put the labels in the queue file itself, and the board can apply the rule on its own.
- The queue page's project column showed "unset" for 76 items that had in fact been given a project. Each of them wrote its project where the filing rules now ask new items to write it, inside the item itself, but the page only looked in the older board-wide list, which new items are no longer added to. So the number of unplaced items grew by one with every new filing, and none of them was actually unplaced. The page now reads the item's own project when, and only when, the board-wide list has no entry for it. Where both places give an answer, the page still shows the one it showed before: which of two disagreeing answers should win is a separate decision, and this change does not make it. On the live board the unset count goes from 76 to 0. A test holds both halves: the item's own answer is used when it is the only answer, and it does not override the board-wide one.

### Changed

- The developer's private queue page was rebuilt around the queue itself, on the owner's call that it had grown long, redundant, and out of step with how the queue actually works. The page used to render a generated summary board whose largest section — around two-thirds of all items — was deliberately just a count, so the one number that mattered most had no rows behind it, and it wore the name "Backlog", which is not a state the queue has ever had. The page now reads the queue directly, from the same tamper-evident source it already checked itself against, and every state in the queue's own vocabulary is a section under its own name — to-do included, every item openable. Each item is one line: its id, its title, and small tags saying what is known about it — whether it has a written spec, when it matters and on what evidence, which project it belongs to, and whether the overnight run has marked it as merged-but-not-yet-confirmed. Those run markers get their own box at the top of the page, because watching them is what the page is for at night; until now they were invisible inside the "active" section. Opening a line shows the item's own "done when" sentence and what blocks it, then loads the full text on demand — the whole queue's text is around three megabytes, which is why it does not ride along up front. Where an item is deliberately excluded from a count the page still lists it, marked, and says why; the summary board still exists for other tools, and if it ever falls behind the queue the page says so while no longer being affected itself. Nothing was deleted to make the page shorter — it got shorter by listing rows once, plainly, instead of explaining its numbers in paragraphs.
- The queue page now takes each item's project and timing from the same place the other planning tools do. Until now four tools read the board, and each one had its own idea of which copy of a fact to believe: this page, the checkpoint's own count, the ranking of what to do next, and the format check. So the same board gave four different answers. There is now one shared reader, kept with the private planning files, and all four ask it. When an item's own entry and the board-wide list disagree, it applies the owner's rule: first an answer that quotes the words it was decided from, then the item's own entry, then the board-wide list. An answer of "unknown" never wins over one that names a value. The page still shows every disagreement, now with the answer it used next to both. On the live board all four tools print the same counts, and the unset count stays at zero. Six ideas their own section describes as "not approved, not scheduled" move out of the working count. A small line in the Horizon section prints those counts in a form that can be compared, character for character, with what the other tools print. On a copy of the project without the private planning files, the page works exactly as before.
...
- Added a safety check that refuses to save a source file the version-control system can only see as raw data. A file written with certain invisible characters in it stops being readable as text: everything still works — it runs, it passes every test — but the record of _what changed_ is replaced by a note saying how many bytes moved. This project reviews changes by reading them, so that is new logic entering the codebase that nobody can read, with nothing anywhere saying so. It happened once on the behind-the-scenes tooling and was caught only because someone happened to glance at a summary line before saving. The check now refuses it automatically, names the file and explains how to fix it, and deliberately ignores genuine binary files like icons, fonts and compiled components — including every one the project already has, so it cannot go off on things that were always fine. Building it turned up a second gap worth its own line: the pre-save checks are installed as a _copy_, so adding a new one does not switch it on until the dependencies are next installed — which meant this very check would have been switched off and silent about it. That is now checked too, and says so loudly instead. No effect on anything you see or do in the app.
