## [Unreleased]<!-- Cache: robco-terminal-v2.8.5-r2 -->

---

## [v2.8.5] — Foundations & Fidelity<!-- Date: 2026-07-22 | Cache: robco-terminal-v2.8.5-r2 -->
...
- The plain, flat "schematic" list in Security & Configuration — the no-frills alternative to the hardware bay, for when you just want a list of switches — now actually lists everything. It had quietly fallen behind as the hardware boards grew: your fourteen individual sound channels were reduced to a single dead line of text that told you to go back to the other view (and it had the count wrong, because the number was typed in by hand rather than counted), and your AI access key, engine picker, handshake button, campaign-log export and app installer had no entry at all — so if you'd left the terminal on the flat view, you couldn't reach your own access key without switching back. Every one of those is now a real, working row in the list, and the sound channels are counted from the actual hardware, so the list can never fall out of step again.
- The flat schematic list now speaks in each machine's own voice. On a Fallout: New Vegas campaign it presents itself as a RobCo field service schematic for a salvaged unit; on Fallout 3 it's a Vault-Tec maintenance diagram for the Pip-Boy 3000, framed like a moulded Vault-Tec document. The controls, their names and their slot numbers are deliberately identical in both — they refer to the same real hardware, and renaming them per game would just make the two views disagree with each other.
...
- The Director can no longer silently delete items you're carrying. Before, a turn where the AI reported an empty or shortened inventory — which could happen after something that changed nothing, like a failed armor repair or an aborted craft — would quietly wipe items you actually owned, with no warning and no way to undo it (one campaign lost a looted 10mm pistol exactly this way). Now the AI's inventory list is treated as a proposal, not the final word. Anything it adds still appears instantly, so picking up loot during the story works exactly as before — but anything it tries to remove or reduce now asks you to confirm first, using the same kind of confirmation popup the terminal already shows before a level-up. Choosing KEEP ITEMS leaves your inventory completely untouched, and any item the AI simply doesn't mention is always left exactly as it is.
- Extended that same protection to everything else the Director could once quietly erase: your companions, perks, quest log, active effects, ammo, and your collectible, trait, skill-book and magazine progress. Before, a turn where the AI sent a short or empty list for any of these could silently wipe it — losing a companion, an earned perk, or your whole ammo stockpile with no warning. Now each of those is treated the same way as inventory: anything the AI adds appears instantly, anything it doesn't mention is always kept, and the only change that can shrink your progress is a perk-rank reduction, which now asks you to confirm first. Two things stay deliberately different because they change constantly during normal play — ammo counts drop every time you fire, and status effects wear off on their own — so those still update freely without nagging you to confirm; they simply can't be wiped all at once by a bad turn anymore. Faction reputation was already safe and was left as-is.
- The AI Director now addresses you by the right character name for the game you're playing — the "Lone Wanderer" in a Fallout 3 campaign, instead of the New Vegas "Courier" it used to call everyone regardless of game. The removal-confirmation pop-up uses the correct name too.
- A message that fails to send and automatically retries no longer floods the screen with a fresh copy of your message and a new status line for every attempt. Your message now appears once and gains a small extra relay-hop marker as it retries, with a single status line underneath counting the attempts (1/3 → 2/3 → 3/3) — replaced by the reply on success, or by the error if it gives up.
...
- The downloaded campaign log is now a truthful record of what was actually on screen. It used to save only the conversation text and silently leave out every pop-up — including the confirmation dialogs the terminal raises before it changes anything. That made the log genuinely misleading: a log could look like the AI had quietly done something drastic when it had in fact asked first and been told no. Pop-ups and confirmations now appear in the log in the right place, with the answer you gave, in all three download formats.
- The terminal no longer greets you as the wrong character on wake. The "COURIER RETURNED" message printed the New Vegas protagonist's name in every campaign; it now uses whichever character your campaign is actually about.
- Choosing the flat schematic list now genuinely sticks. The terminal was correctly writing down which of the two views you last picked, and then ignoring it on the next startup — so if you preferred the flat list you were handed the hardware bay again every single time you reopened the app. Picking a view was effectively pointless for anyone who kept the Security & Configuration panel open. Your choice is now honoured on startup, whichever one you picked.
- The print-rate slider in the flat schematic list was only four pixels tall — a nearly impossible target to hit with a fingertip. It's now a full-size control, along with the access-key field, which sat one pixel under the terminal's own minimum touch size. The tick boxes in that list were also below the minimum and are now full-size and phosphor-green rather than the browser's default blue.
- Dragging the print-rate slider in the flat schematic list no longer leaves the hardware bay's copy of that slider showing the old position until you reload. The two views now always agree, in both directions.
...
- Retired an internal bookkeeping rule that required the exact number of automated tests to be hand-copied into eight different documentation files every single time a test was added or removed. The number described how many checks exist, not whether any of them passed, so keeping it in step protected nothing while making it more expensive to do the right thing and add a test. The build check that policed the number is gone with it. Historical release records keep the test count they shipped with — that is a record of the past, not an ongoing chore. Nothing about how thoroughly the app is tested changed; the full suite still has to pass cleanly before anything ships.
- Wrote down, for the first time, how a project rule or automated check is allowed to be REMOVED — previously the process could only ever add more. A rule can now be retired when the problem it guarded against no longer exists, on the condition that its enforcement is deleted alongside its description and that any protection genuinely lost is stated plainly rather than glossed over. It cuts both ways: the check that holds the app's internal structure together is explicitly marked as NOT eligible for retirement, because the risk it covers is still live.
...
- When the automated checks fail during an update, the terminal's build system now saves a snapshot of exactly what went wrong — a screenshot of the screen at the moment a browser check failed, that screen's console messages, and a per-check log naming the failing tests — and attaches them to the failed run. A red run can now be diagnosed straight from that saved evidence instead of having to re-run the whole check locally just to see what happened. Proven by deliberately breaking a check and confirming the screenshot and logs were actually captured, then undoing the break. Only saved when something genuinely fails; a passing run saves nothing. Nothing you can see changed.
...
- Strengthened three more of the safety checks that guard your saved campaign so they run the real code instead of just scanning it for the right words. The map's "you've been here" memory is now proven to actually work — it collapses the same place logged twice (in any capitalization) into one entry, keeps every place you've ever discovered without ever quietly trimming the list, and ignores blank names — where the old check only confirmed the words were present and could have stayed green with the memory silently broken. The same was done for the cargo screen's quantity steppers and the new equip control: the tests now run the real code to prove a quantity can't go negative, that hitting zero removes the exact item you were adjusting (not a different one), and that equipping a weapon or armor swaps it into the right slot, un-equips on a second tap, and correctly ignores an item that can't be equipped. Each new check was proven to genuinely catch its bug by breaking the real behavior in a way the old word-scan couldn't see, watching the test fail, then undoing it. Nothing you can see changed.
- Strengthened three more of the offline-command safety checks the same way — running the real code instead of scanning it for the right words. The "[SLEEP]" and "[WAIT]" typed commands are now proven to actually rest you: sleeping advances the clock by the correct eight hours, refills your health, and heals every limb, and both commands are proven to refresh the screen before saving so the change can't be quietly undone. The barter terminal's buy and sell prices are now checked by running the real pricing on real numbers across the whole barter-skill range, where the old check only re-did the math a second way and never actually called the shipped code — so a wrong price could have shipped unnoticed. And the quick-log shortcuts you type in TERMINAL mode ("killed 3 raiders", "+50 caps", "arrived Novac", "rep NCR up") now have their real effects verified: the kill is logged with the right wording, caps are added and mirrored correctly (and can't go negative), a location is recorded trimmed, and a faction's standing moves the right way — while an unknown faction is correctly ignored rather than invented. Each new check was proven to genuinely catch its bug by breaking the real behavior, watching the test fail, then undoing it. Nothing you can see changed.
- Strengthened the safety check guarding the offline V.A.T.S. combat calculator the same way — running the real math instead of scanning the code for the right words. The calculator's hit-chance, action-point, and damage numbers were previously tangled up with the on-screen drawing, so the check could only confirm the right terms were present; a flipped sign, a wrong cap, or a broken formula could have shipped a wrong number unnoticed. That math is now a separate, self-contained piece the tests actually run — verifying exact hit percentages (including the top and bottom caps), action-point pools, per-body-part strike counts and the best target to hit, effective damage after the target's armor (never dropping below one), and the sensible fallbacks when no weapon is equipped or a game supplies no data of its own. The calculator itself looks and computes exactly as before — this was a pure internal split with no change to any number it shows. The new checks were proven to genuinely catch their bug by breaking the real math (a flipped action-point formula, a flipped damage-vs-armor sign), watching the right test fail, then undoing it. Nothing you can see changed.
- Strengthened three more of the safety checks the same way — running the real code instead of scanning it for the right words. Editing a S.P.E.C.I.A.L. attribute is now proven to actually clamp your entry to the real 1 to 10 range (an over-max value snaps to 10, a zero or negative snaps to 1) and, crucially, to fall back to your previous value rather than a hard 1 when you blank the field out — the exact snap-to-1 mistake the old word-scan couldn't have caught. The safeguard that stops a background save from overwriting a campaign you just switched games into (or just loaded from a slot or the cloud) is now proven by actually running the save step and confirming it holds its write while the guard is up and resumes normally once it clears — where the old check only confirmed the guard's keywords were present and would have stayed green even if it checked the wrong condition. And deleting an item, status effect, perk, or field note is now proven to update your campaign before the save fires, not after — the old ordering mistake that would have quietly saved a stale copy — with the on-screen refresh confirmed to run first and an out-of-range delete confirmed to do nothing at all. Each new check was proven to genuinely catch its bug by breaking the real behavior in a way the old scan couldn't see, watching the test fail, then undoing it. Nothing you can see changed.
- Strengthened the save-file and cloud-save safety checks the same way — running the real code instead of scanning it for the right words. Exporting your campaign to a file and importing it back is now proven to survive the round trip whole: the real export builds the file, its built-in integrity seal is checked against a fresh recomputation, and feeding that exact file back through the real import restores your character, chat history, and playstyle unchanged — the disaster-recovery path no earlier check actually ran, and it also confirms the exported file is shaped for the modern import path so the two halves can never quietly drift apart. And the cloud-save safeguards that protect you when you overwrite or delete a cloud save are now proven by actually running them: an overwrite is confirmed to archive the previous contents into version history before it replaces them — and to capture the about-to-be-lost data, not the replacement — and a delete is confirmed to clear every retained earlier version before removing the save itself, so no leftover history is ever orphaned. The old checks only confirmed those steps appeared in the right order in the code and couldn't tell whether the archive actually grabbed the right data. Nothing you can see changed.
- Measured, for the first time, exactly where the automated pre-release checks spend their time — the build now prints a per-step timing breakdown at the end of every run. It confirmed that two thorough browser checks (the screen-integrity sweep across twelve screen/game combinations, and the save-survival boot test across four save files) account for roughly nine-tenths of the wall-clock, and that the rest is already trimmed as far as it safely can be — the checks already share a single browser instead of relaunching one each. The remaining time is genuine coverage, not waste, so nothing was cut: shaving it would mean either dropping coverage or loosening the settle-timing that keeps those checks from flaking, neither of which is worth it. Nothing you can see changed.
- Made the build's failure-snapshot screenshots reliable on the slower cloud test machines. When a browser check fails, the build grabs a full-page screenshot as evidence — but the terminal's screen animates continuously (the scan-lines and the AI presence dial both repaint every frame), and capturing a full-page shot of a constantly-moving screen could stall on a slow machine and waste the whole capture window. The snapshot step now briefly freezes the screen's motion before it captures, so the shot is taken against a still frame every time. This only affects the diagnostic screenshots saved when something is already failing — it never changes what any check tests — and a new automated safeguard locks the freeze in so a future change can't silently drop it. Nothing you can see changed.
- Measured, for the first time, exactly how the terminal loads on a phone — start-up timing, every file it downloads, and where the main work happens. The finding: the app's own code is already lean. It paints its first screen almost immediately, every one of its own files loads and runs in a couple of milliseconds, and once installed they're all served instantly from the offline cache, so they aren't what you wait on. The heavier warm-up — the background sign-in and cloud-sync connection to Google — runs quietly after the terminal is already up and never delays it starting (the start-up sequence you see is the deliberate boot animation, not that connection). Because of that, no change could be made here that was both a genuine speed-up and completely safe, so nothing was changed rather than risk how a feature behaves. The two real opportunities found — holding that background cloud/sign-in warm-up until you actually use cloud features, and only drawing the visible rows of very long lists — each change real behavior and are noted for their own dedicated, separately-verified passes later. Nothing you can see changed.
- Swept the entire terminal for any hidden reliance on the internet in the parts that are supposed to work without it, and confirmed the offline promise is airtight. Every part of starting up, every screen, and every on-device tool loads only from files stored on your own device — none of them reach out to the network. The only features that use the internet at all — the AI Director, cloud saves and sign-in, and the on-device screenshot scanner's optional AI fallback — are cleanly walled off: they load quietly in the background, never hold up the terminal starting, and every one already checks whether it's actually available before it runs, so the terminal stays fully usable with the connection cut. Also confirmed the offline cache covers every file the offline core needs (all 47 of them), and that the automated checks already fail the build if a new core file is ever added without being cached — so a fresh accidental dependency can't slip in unnoticed. A stricter, permanent "no new outside address" guard is a deliberate follow-up for a later dedicated pass. Nothing you can see changed.
- Ran a full dependency and security-hygiene check over the terminal, and it came back clean. The development toolchain reported zero known security vulnerabilities. The two pieces of outside code the live site loads are both locked to a fixed, known version — the Google sign-in and cloud-save library is pinned rather than tracking whatever's newest, and the screenshot-scanner text-recognition engine is stored on your own device rather than fetched from anyone else's server — so neither can silently change underneath you. Also spot-checked that anything you type, anything the AI sends back, and anything imported from a save file is cleaned before it's shown on screen, so a booby-trapped item name can't run code in your terminal; the paths that display that content all passed. Nothing you can see changed.
- Did a final housekeeping pass over the terminal's own code, looking for anything left over — dead or unreachable code, stray developer print-outs, commented-out fragments, unfinished-work markers, or small duplicated helpers — and it came back clean. The earlier large reorganization had already swept it: the automated code checker (which flags any unused value, unreachable branch, or empty block) is green, there are no leftover debugging print-outs or to-do notes in the shipped code, no commented-out code, and the one text-safety helper is written once and reused everywhere rather than copied around. A couple of deliberately-parked values for a future Fallout 4 build are clearly labeled as such and were left untouched. Nothing you can see or interact with changed.
- Turned a long-standing internal design rule into one the build now actually enforces. The rule keeps the terminal's code in clean layers — the screen-drawing code only draws, the code that saves your campaign is the only thing allowed to save it, the game-data files are read-only, and the online-services code isn't allowed to reach up and redraw the screen. That rule was previously honor-system only, and over time the code drifted: the screen-drawing files write your save in about twenty places, and the online-services files reach up and redraw the screen in about twenty-six places. A new automated check now measures exactly those crossings and locks in today's counts as an accepted starting point, so the tally can't grow — any brand-new crossing fails the build and names the file and rule — while the existing ones are left in place to be untangled properly in a later dedicated pass. It was proven to work by deliberately adding one new crossing, watching the build fail and name it, then undoing it. Nothing you can see changed.
- Made backing up the development notes and design files automatic. Those files only ever live on the developer's own machine — they aren't part of the public project, so if the machine were lost they'd be gone for good; a small private backup copy exists for exactly that reason. Keeping it fresh is now the development tooling's own job rather than something anyone has to remember: whenever those files change, the working session is prompted to refresh the backup itself, and if it ever can't, it has to say so rather than skip it quietly. The prompt can never delay, block, or interfere with publishing new work, and on any machine without the private backup it stays completely silent. Nothing you can see changed.
- Repaired the safeguard that reminds the developer to refresh the terminal's "update" stamp whenever a file that reaches your device changes. That stamp is what tells an already-loaded terminal a newer version exists, so forgetting to move it means you silently keep running the old build. Because of the way day-to-day work is now organized, the check had quietly stopped doing anything — it was comparing against the wrong reference point and always came back happy no matter what. It now compares against the correct one, so it genuinely catches a forgotten stamp again on every branch, while still stepping aside for changes that never reach your device. Its own test was rewritten to actually run the safeguard and prove it both catches a mistake and passes a correct change, rather than just checking that the code looked right. Nothing you can see changed.
- Tidied up three pieces of the terminal's internal records that had drifted out of line with reality: an automated check that was quietly waving through one deliberately-reserved item is now honest about doing so; a leftover message told the developer two test runners still ran when one had been retired weeks ago; and an internal file map listed wildly outdated file sizes that contradicted themselves elsewhere in the same document, so those figures were dropped entirely rather than patched (they were the kind of hand-kept number that just rots again). The project's own rulebook also gave contradictory instructions about which branch work goes to; those were made consistent, keeping the strict "confirm it's really live" step for public releases intact. Nothing you can see changed.
- Stopped instructing the AI to resend your entire inventory on every item-related turn — it now reports only what actually changed. This both removes the root cause of the silent item-deletion bug fixed above (a full list could be short or empty by mistake and overwrite everything) and trims how much data is sent on those turns, which lowers the per-message cost for anyone playing with their own AI key. A new automated safety check proves an AI turn that returns an empty or shortened inventory can never delete items you natively hold — proven to fail against the old code first, then pass against the fix — with the whole confirm-before-removing flow locked in the same break-then-fix way.
- Applied that same "report only what changed" instruction to perks, quests and faction standing — the AI used to be told to resend those whole lists every turn too, the exact instruction that made the deletion bug possible. Reporting only what changed removes that risk at the source and further trims the per-message cost on your own AI key. A regression test for every one of the now-protected areas was added — each proven to wipe data against the old code first, then keep it against the fix — plus a permanent build guard that fails the project's tests if anyone ever re-introduces the old "overwrite the whole list from the AI" shortcut for any of these fields, so the bug can't quietly come back one field at a time.
- Made the AI's player-character name a per-game data value instead of hardcoded text, so a future game only needs a new data entry rather than scattered text edits — this is what fixed the wrong-character-name issue above. Added regression tests covering the retry display collapse, the network-failure severity split, the wake-message de-duplication, and the ambient-chatter throttles, plus a permanent build guard that stops the confirmation buttons from ever silently reverting to the old blue-rectangle style, and updated the AI-instruction fingerprint test to lock in the corrected per-game wording.
- Audited the instructions the terminal sends the AI on every message against what the app now actually does for itself. Over time the calculators, the trackers, level-up, trading, item use and the map all became built-in and offline, but the AI was still being told it was in charge of them — instructions you pay for on every single message, and an invitation for it to send back data the app already owns. Five stale instructions were removed, including one that taught the AI an experience-point formula that no longer matched the app's own. The example the AI is shown also used to demonstrate resending every faction at once, directly contradicting the rule right beside it telling it to send only what changed — it now shows a single changed faction. The result is about 500 fewer characters sent on every message. Instructions the AI genuinely still owns were deliberately left alone and are now pinned by a test so a later pass can't strip them by mistake.
- Added regression tests for all of the above: that an AI change never switches tabs, that the change cards queue instead of overwriting, that the exported log places pop-ups in the right position with the recorded answer, that trimming old conversation lines doesn't shift the recorded pop-ups out of place, and that the wake message follows the active game.
- Removed a dead recipe table from both games' internal item databases. Crafting has always read its recipes from a separate, properly structured list, so this table fed nothing on screen — but it was still being sent to the AI with every single message, costing you tokens to hand it a second, competing recipe list for a job the terminal now does entirely on its own. Deleting it also cleared out an invented Fallout 3 recipe that produced an item which doesn't exist in the game. Nothing you can see changed, and crafting is completely unaffected.
- Strengthened the safety check on the game-cartridge picker so it now actually renders the cartridges and inspects the result, instead of just reading the code and assuming. The old check claimed to prove that a game's name is safely escaped before being shown, but it could only confirm the escaping was _written_, not that it _happened_ — so a future rewrite could have kept the check passing while opening a real hole. It now feeds in a deliberately malicious game name and confirms it comes out harmless.
- Reorganized the development rulebook the AI assistants follow so they load only the rules that apply to what they are actually changing. It used to be one very large document every session read from top to bottom no matter how small the job, which meant the important rules competed for attention with dozens of irrelevant ones — twice already that let genuinely contradictory instructions sit side by side unnoticed. It is now a short set of rules that always apply, plus ten focused notes (saves, service worker and deploy, sign-in and cloud, screen layout and mobile, sound, game data, the AI contract, file layout, tests, and documentation) that are pulled in only when that part of the app is touched. Nothing was removed — every rule still exists, in exactly one place — and three new automatic checks now fail the build if a rule gets duplicated into two files, if a note goes missing, or if the boot-order list drifts from the real code. Nothing you can see changed.
- Trimmed the first small batch of rules out of that same development rulebook, now that it is organized well enough to see what was actually pulling its weight. Two rules that only repeated advice the assistants already receive elsewhere were retired outright, and two step-by-step "how to add a new screen panel" checklists were replaced with a pointer to the automatically-derived code map — those checklists had quietly gone wrong, still naming a file that was split into nine some time ago, and a confidently wrong checklist is worse than none. One rule about never running two sessions at once was reviewed for removal and deliberately kept, with the reason written down so it does not get re-argued later. Every retired rule stays on the page as a dated record of what went and why, so nothing silently disappears. Nothing you can see changed.
- Added an automatic check that compares the flat schematic list against the hardware bay and fails the build if the bay gains a control the flat list can't reach. This is the check that was missing: the old tests only confirmed that a handful of named switches were present, never that the list was complete — which is exactly how whole boards came to be missing from it without anything going red. Any control deliberately left out now has to be named, with a reason, in the check itself, so an omission is always a decision somebody wrote down rather than drift nobody noticed. Nothing you can see changed.
- Cleared the last two loose ends the rulebook reorganization left behind. A second rules file had been reduced to a 32-line note whose only remaining job was to say "go read the other one" — it is now deleted, and the handful of automatic checks that pointed at it were each re-aimed at the thing they were really protecting rather than being thrown away with it. Separately, the per-section test counts written throughout the test file were stripped out: an earlier change removed the requirement to keep those numbers accurate but left the numbers sitting there, so they still read as fact to anyone opening the file — the chore was gone but the misinformation wasn't. Those same stale counts were cleaned out of the internal reference documents too, and a new check now fails the build if either habit creeps back. The one count that genuinely earns its place — the test page's own tally of how many sections it runs, which checks the page against itself — was deliberately left alone. Nothing you can see changed.
- Changed a development rule that had quietly been making the tests weaker. One rule instructed every session to guard important behaviour by checking that the _source code looks right_ — and sessions dutifully followed it, so the safety net filled up with tests that confirm the code _says_ it does the right thing rather than that it actually does. That is not a small distinction: one such test was found this week checking only that a screen _claimed_ to make user-typed text safe to display, which would have passed just the same had the safety been broken. The rule now says the opposite by default — prove it by actually running the thing — and reading the source is allowed only where genuinely running it can't reach, with the reason written down at the test. The handful of places where reading the source is the only sensible option (checking files use the right text encoding, checking a deleted file stays deleted, checking the startup order) keep working exactly as before. Nothing you can see changed.
- Wrote down where a reviewing session's evidence has to come from. The project's workflow ends every job with an independent review, but it never said what that review is performed _against_ — so reviews were being done by reading the working session's own write-up of what it did, which is not evidence of anything, and were sometimes skipped entirely (the write-up always says the work is finished, so nothing looks wrong). The rule now states plainly that the review is triggered by, and carried out against, the actual code changes, and that it cannot be marked as passed unless the reviewer names what it actually read. Nothing you can see changed.
- Put the outstanding Firebase "App Check" security work on the roadmap in writing, including one dependency that had only ever been said out loud: a debug credential that can bypass that protection was found stored in the project's private history archive. Nothing is exposed today — that archive is private and the credential is a development-only one, and the app itself stores no such credential — but it has to be replaced before that archive could ever be made public, which means it now blocks the planned public "museum" of the project's history. The roadmap now carries that dependency explicitly rather than trusting anyone to remember it. Nothing you can see changed.
- Followed a folder reorganisation through every place that still pointed at the old locations. The project folders on the owner's own machine were renamed, and several behind-the-scenes tools still had the old paths written into them — including the script that backs up the project's local-only notes and planning documents, which would have refused to run. The backup tool, the reminder that prompts it, the history-museum generator, and the project's own rulebook now all point at the new locations, and the backup's link to its private storage was corrected too. The app itself is untouched: its public address is deliberately unchanged, so nothing about the installed app on anyone's phone is affected. Nothing you can see changed.
- Went through the project's private reference notes — the ones future work is planned from — and checked every claim in them against the actual code instead of trusting them. They had each been written at a different time, so they had quietly drifted apart on the same facts. Several were genuinely wrong: one pointed at a piece of the app that had been renamed and no longer exists, and its shortcuts for finding things were off by as much as several hundred lines, so they've been removed in favour of just searching by name. Another still described an older version of the project's own rulebook and, more seriously, had never recorded the fix that stopped the AI from wiping items out of your inventory — the notes are what future work is planned from, so a missing safety fix there is the kind of gap that lets a bug come back. A third contradicted itself about whether it tracked test numbers. All of them now say which version of the project they describe, so anyone reading later can tell at a glance whether they're still current. A gap in the project's own safety checks turned up during the pass and was closed: one check had been silently examining less than it claimed to while still reporting success. Nothing you can see changed.
- Fixed the project's reusable "quick-start" instruction pack — a short primer a work session reads before it even opens the project — which had drifted badly (it named a folder that no longer exists, a testing tool that was deleted, and an out-of-date rule), because it lived only as an installed copy nobody could review or correct. It now has a proper home inside the project's own files, and — the key change — it was rewritten to be a **pointer** to the real rules ("read the rulebook, here's where to look") instead of a second copy of them, so it can't silently disagree with them again. The build's existing documentation-integrity check now also covers it, catching the exact kind of dead reference that caused the drift, with no new machinery added. Nothing you can see changed.
- Wrote down plainly, in the project's rulebook, one thing the recent "make sure plans get written down" safeguard genuinely cannot catch: a decision made and acted on entirely within a single conversation that never gets saved as a note. An automated reminder can compare saved notes against the plan list, but it can't read a conversation, and the sessions most likely to forget never reach the point where the reminder runs — so this is honestly marked as a habit to keep, not a thing a tool can enforce, rather than pretending a checkbox covers it. Nothing you can see changed.
- Added an opt-in safety check for the cloud-save path (`npm run cloud-check`). Cloud sync stores your whole campaign as one record, and there was a blind spot: if a future change added a new piece of saved data in a shape the cloud database silently drops or rejects, nothing would have noticed. The planned way to catch that is a full test against Firebase's local test server, but that server needs a Java runtime that isn't installed on the owner's machine yet — so this is the honest interim: it reads the real list of saved fields straight from the code (so a newly-added field is checked automatically, not from a list someone has to remember to update) and flags any that the cloud database couldn't store faithfully. It was proven to work by deliberately breaking one field and watching it fail, then fixing it and watching it pass. It's a smaller safety net than the full test — it checks the shape of your data, not every possible value, and doesn't stand in for the real database — so the bigger test stays on the list for when the Java runtime is installed. Nothing you can see changed.
- Promoted that cloud-save safety check from opt-in to automatic — it now runs as part of the standard pre-release checks on every single change (both the quick check made when work is saved and the fuller one made before it's published), instead of only when someone remembered to run it by hand. It's cheap enough to belong there: it reads the list of saved fields straight from the code, with no database, browser, or internet involved, so it sits alongside the other fast checks and adds no real time. A safety check that runs on every change actually catches a mistake the day it's made; an opt-in one nobody remembers to run does not. It still refuses to pass quietly if it can't read the field list (so it can never look green by doing nothing), still double-checks itself against a known-bad example on every run, and stays runnable on its own too. Nothing you can see changed.
