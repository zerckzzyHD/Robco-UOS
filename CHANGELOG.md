...
- Big moments in your campaign now come with feedback you can actually see. Leveling up pops up a Vault-Boy card with a radial sunburst behind it. A faction standing crossing into Vilified or Idolized slams down like a rubber ink stamp. Completing or failing a directive punches a stamp onto its slot on the quest rack. Discovering a new location pings the map with an expanding radar ring the next time you look at it. Radiation climbing into a new danger tier makes the RAD gauge chatter with a flash of film grain; a crippled limb flashes bone-white with a fracture line drawing itself across it. A newly collected curio lights up its display case like a spotlight switching on, and critical HP now shows as a genuinely failing heartbeat trace with a breathing red glow at the screen's edges. If any of these happen while you're looking at a different screen, a small readout at the very top of the terminal announces what happened — tap it to jump straight to the screen that reacted. Several happening back to back collapse into one line with a count instead of piling up, and the readout keeps showing even with reduced motion or a quieter Immersion setting turned on.
- More moments now get their own bit of feedback. Taking damage rips a single flicker of static across your vitals monitor, and gaining XP brightens the new sliver of your progress bar with a floating "+N XP" note that drifts up and fades. Adjusting a S.P.E.C.I.A.L. stat makes its gauge overshoot and settle back into place with a click, its letter flashing bright for a moment. Marking a skill book or magazine as read drops a rubber stamp onto it that thunks down slightly rotated with a couple of ink specks. Your caps readout now rolls like an odometer on any change, with a bottle cap arcing into place after a trade. Adding an item to your inventory slides its row in and pops the hole-punch on its tag, blipping the drawer's count. Finishing a craft sparks briefly at the bench before a claim tag types out the item's name and tears free. Sleeping collapses the screen into a bright line while the mission clock's date/time drums spin, then blooms back awake. Saving slides a small holotape into place on your Save Archive with its reels spinning, and a cloud save adds a ripple to show the signal went out. As with the earlier batch, a small readout announces any of these if you're looking at a different screen when they happen.
- The last batch of these little touches rounds things out. Healing refills your vitals trace with a bright leading edge and a small medical "+" blinks beside your condition word. Getting overloaded gives the weigh bridge an actual clang and shudder as it seizes up. Socketing a new perk drops its card into the rack with a bounce and a spark on its pins. Nudging a faction's fame or infamy kicks the reputation needle past the new mark before it settles. Crossing into a new karma tier bounces the needle and blinks a small halo or horns beside your standing. Equipping a weapon or piece of armor slaps an "in service" stamp onto it. Scrapping an item drops its parts into the yield list with a little bounce. Filing a new quest slides its slot in and warms its lamp from dark to lit. Arriving somewhere sweeps two lines onto your position on the map. A status effect coming on warms in like a tungsten filament; one about to run out flickers like a dying bulb until it does. A recorded fault gives the warning lamp a quick flicker, and nudging an item's quantity flips its digit like an odometer. As with the earlier batches, several of these also show up in the small readout at the top of the terminal if you're looking at a different screen when they happen.
...
- Starting a new campaign now runs a short commissioning sequence — the screen dips, a few calibration lines type on, and the Director's own greeting arrives as your first line — instead of two plain reset lines. It only takes a couple of seconds, and tapping or pressing any key skips straight to the end if you're in a hurry.
- The Director now greets you the moment you first open the Uplink each session, instead of staying silent until you send the first message yourself.
- The first time you boot up after an update, the terminal now calls it out with an extra line during startup, a brief glint on the casing nameplate, and a highlighted revision-log button pointing you at what's new.
- Coming back to the terminal after being away for a few days now gets a small "recalibrating" line during startup noting how long it's been. Booting up normally, day to day, stays exactly as quiet as it's always been.
- Reseating a Module Bay board, picking a phosphor tube, seating a program cartridge, or opening the Tool Deck now gives a quick, satisfying physical settle — a brief brighten-and-fade — matching the tactile click/thunk those already had.
...
- Your Campaign Configs now look and feel like real hardware, matching the rest of the settings section. Which game you're playing shows as two cartridges you tap to seat, and your playstyle is a two-way switch. Everything still does exactly what it did before, just faster to use and easier to read at a glance.
- Your playthrough type is now picked with a real rotary dial, centered where you'd expect a knob to be. The five options sit around it, and the dial's needle always points straight at whichever one is active. Drag the knob to spin it, tap a position directly, or use the arrow keys — whichever is fastest for you — and the full name and a short description of the active choice always shows just below the knob so it's never hard to read. Tapping the knob itself doesn't do anything on its own; it's the drag, the direct tap, or the arrow keys that make the pick.
...
- Your OPERATOR screen got a full hardware makeover to match the rest of the terminal. Your health, level, and radiation now read out as glowing patient-monitor traces with a bottom strip showing your caps, carry weight, and AP alongside a LEVEL UP key that flashes "LEVEL N COMMITTED" when pressed — and the HP and XP/GRADE traces are both draggable, not just typeable, so you can set either one by dragging right along the bar. Your S.P.E.C.I.A.L. stats are now a row of seven sliders with lit segment ladders and dedicated plus/minus buttons — and each slider is draggable up and down, in addition to typing a number or tapping the steppers. Your limb condition shows as a tappable anatomical diagram that blinks red wherever you're hurt, mirrored by the same limb buttons as before, and tapping a limb on the diagram itself toggles it crippled or healthy just like the buttons do. Every other board on the screen — perks, status effects, faction standing, karma, and your skill list — now shares the same labeled hardware look for a consistent feel. Nothing about how any of these work has changed: every field is still editable exactly as before, and all your existing buttons do exactly what they always did.
...
- Your CHASSIS section got a full hardware makeover to match the rest of the terminal. Your device status is now split across three labeled boards: a UNIT POWER PLANT showing your session and lifetime uptime as industrial hour meters beside a pulsing power core, an IDENTITY PLATE & BREAKERS board showing your firmware and connection status as a stamped nameplate with breaker switches, and a SERVICE & FAULT CONSOLE holding the firmware revision log and error log buttons. The power core is alive — it beats gently while the terminal sits idle, spins up and glows brighter while the Director composes a reply, dims and flickers if your connection drops, dims and slows when the terminal is idle or in standby, collapses to a dark point when fully shut down, shows a faint red ring whenever an error is buffered, shimmers while the Pip-Boy Radio plays, flashes once on a level-up, pulses on every save, glows in whatever optic color you've picked, and works visibly harder when several of those are happening at once — every one of these reactions was brightened up so they're easy to catch at a glance, not just on close inspection. Tap the core for a quick, purely cosmetic kick, or tap its "?" for a plain-language explainer of what each behavior means. A mirror of the same core also sits in its own readout window built into the terminal's top casing, right alongside its PWR/UPLINK/FAULT lights — that window and the core inside it have been made noticeably bigger twice over now, and the bigger phone screen sizes scale down cleanly on narrower phones — so you can see it react from there too.
- Using an item from your cargo manifest (a Stimpak, RadAway, a chem, food, and so on) now applies its actual effect right away and fully offline — healing, removing radiation, mending a crippled limb, clearing an addiction or poison, or picking up a timed buff — instead of sending a message to the Director and waiting for it to narrate the outcome. The USE button now only appears on items that can actually be used; a message in your log confirms exactly what happened.
- Visual Upload now reads your screenshot right on your own device by default — tap the attach button and it scans the image itself, no round trip to the Director needed, and it works fully offline once you've used it at least once before. It still shows you exactly what it found on a review screen for you to correct, adjust, or leave out, and nothing is added to your campaign until you confirm. If the on-device scan can't run, it quietly hands off to Director vision instead — or you can ask for Director vision yourself with a TRY AI VISION button if nothing was found. If neither is available, you're told plainly and pointed at CARGO MANIFEST so you can still add items by hand.
...
- Fixed your RAD EXPOSURE reading having no upper limit — it could previously be pushed past the point where radiation is actually fatal. It's now capped at the same value that would kill your character, whether you type it in yourself or the AI reports it.
...
- Fixed a moving line on the map screen occasionally passing over the legend text at the bottom, briefly hiding the words it was explaining. The map's rotating scan effect is now permanently boxed in to the chart itself and can never spill onto the text below it, no matter what device you're on.
...
- Fixed the SAVE TO CLOUD button being easy to overlook on a phone. It now sits directly beside EXPORT SAVE at the same size, instead of alone in a smaller, differently colored row above your saves list.
- Fixed your CAMPAIGN PROFILE summary and the cartridge-swap confirmation always reading your active game's real title. New Vegas now reads "Fallout: New Vegas" to match its title everywhere else in the terminal, and any game other than New Vegas now shows its own actual name instead of always defaulting to "Fallout 3."
...
- The DIAGNOSTIC SHELL can now set up test scenarios on demand — a full heal, maxed-out stats, low-HP or crippled or over-encumbered or addicted or high-level test setups, and dozens of other one-tap state changes across health, S.P.E.C.I.A.L., skills, karma, caps, inventory, factions, collectibles, perks, quests, the map, status effects, companions, and time of day — plus matching one-tap resets (map fog, quests, inventory, stats and counters, and more), including a couple of small reset buttons right on the map, directive, and cargo screens for quick access during testing. A new one-button "Load NV Test Campaign" fills the terminal with a fully-stocked practice campaign for testing and only works while playing New Vegas, since it would otherwise mix up the two games' data. Nested groups of controls inside the panel are now also visually indented under the section they belong to, so the hierarchy is clear at a glance. Developer/staging tooling only — never visible on the live site, and nothing about any existing feature's behavior changed.
- The DIAGNOSTIC SHELL rounds out with its last set of tools: a switch for each remote on/off feature setting (cloud sync, Google sign-in, the AI chat link, key syncing, save migration, the offline save queue, and both halves of on-device screenshot scanning) so its disabled behavior can be tried out on the spot without touching the live configuration — flip it on, off, or back to its normal default at any time. A couple of one-tap buttons simulate repeated AI or screenshot-scan failures to check that the automatic "give up and fall back" behavior kicks in correctly, and a preview option shows what each kind of AI hiccup (an invalid key, a rate limit, a timeout, a garbled reply) would actually look like without needing to trigger a real one. The update-required prompt can now be shown on demand for testing, and there are one-tap buttons to clear all cached files or the offline install entirely. This unit also lays the groundwork for a future in-game hacking minigame: solving it will one day unlock this same panel for a player on the live site — but only ever a safe, read-only sandbox version, with a short "RESTRICTED ACCESS GRANTED" flourish marking the moment. A matching pair of test buttons lets that unlock be rehearsed ahead of time. A thorough final safety sweep confirms every cheat, reset, and raw technical readout in the panel is still completely unreachable outside of testing. Developer/staging tooling only — never visible on the live site, and nothing about any existing feature's behavior changed. This completes the DIAGNOSTIC SHELL.
