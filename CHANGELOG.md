...
### Fixed
- **Conservation of Mass (Crafting Delta)** (BRAIN.md, Dev Manual §2.5): Engine now explicitly calculates and adds the mass of newly yielded crafting items *after* purging the consumed ingredients, closing an inventory mass leak.
...
### Changed
- **Mobile Geometry Overhaul** (BRAIN.md, robco_ui_templates.txt): Replaced all standard keyboard table frames (`-`, `=`, `_`) with seamless Unicode box-drawing characters (`┌ ┐ └ ┘ ├ ┤ ─ │ ┼`) to eliminate micro-gaps.
