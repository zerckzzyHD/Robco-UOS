...
⚠ **It asserts NON-BLINDNESS, not currency** — the one step in the 4x family that does. Protocols
47/52/53 regenerate and byte-compare; this one reads the board's own `ROADMAP-STATE` marker. The reason
is the generator's own design: it fails **closed** (a bad parse yields a BLIND board, never a partial
one) but always exits **0**, because a reporter must never be able to fail a sync or a commit. That
combination is safe and has one cost — a blind board can sit on disk indefinitely, looking like a
document. Step 4f is where that refusal becomes a red.
