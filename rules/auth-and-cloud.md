...
**Part (c) — Protocol 35: flip the flag off FIRST on a detected post-deploy regression — a MANUAL runbook, not an automated actor.** When a post-deploy regression in a networked/IO feature is detected, the flag is flipped to `false` in `/config/flags` **as the first action, before diagnosis** — restore first, diagnose second; the live-site analogue of Protocol 16 for flaggable features. Order: (1) flip off live, (2) confirm fallback active + app usable, (3) report to the user, (4) diagnose/fix/verify, then flip back on. Prefer the remote flag over `git revert` when the break is contained to a flaggable feature (instant, no deploy/cache bump) — the fail-safe read (b) means flipping only disables into the defined fallback, never black-screens.

> **⚠ CORRECTED 2026-08-02 (F06 of the five-repo ground-truth audit) — this protocol used to say the flip happens "immediately and automatically," and NOTHING AUTOMATED EXISTS TO DO IT.** The wording is fixed rather than the claim quietly kept, because "automatically" is the kind of word that stops anyone checking.
>
> **What is actually true, verified in source:**
>
> - **The remote flip is a HUMAN action in the Firebase console.** `firestore.rules` scopes the flag doc `match /config/{doc} { allow read: if true; allow write: if false; }` — **world-readable, client-writable by nobody.** No app code path, no GitHub workflow, and no script in this repo writes `/config/flags`; the only references in `js/` are comments. So the flip is fast (no deploy, no cache bump — that part was always true) but it is **performed by a person, at a console, at human latency.**
> - **The in-app half is a LOCAL, SESSION-SCOPED safeguard only.** `cloud.js`'s Diagnostic-Shell override layer (`robco_dsh_flag_overrides`) shadows the local read _"never reaches the remote config"_ — it is a staging test seam and a per-device fallback, **not** a way to disable a feature for anyone else.
>
> **Therefore the claim this protocol may make is bounded:** _"a flaggable regression can be contained in one console action without a deploy"_ — **not** _"a flaggable regression never waits on a human round-trip."_ It always waits on a human; the point of the flag is that the human's action is **instant and reversible** instead of a release cycle. If genuine automation is ever wanted here it needs a server-side actor that does not exist today, and that is a build, not a wording change.
