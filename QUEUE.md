...
**What it depends on.** The archive repo and its folder structure (exists, Protocol 48). It reads the archive, so it wants the archive current — which the re-pin (R4) and ongoing sync keep true. ✅ **The App Check debug-token blocker on publication is CLEARED (2026-07-20)** — all three tokens were deleted in the Firebase console and the dead string is out of the archive, so publishing is no longer gated on it. See the App Check entry in the 2.9.0 round.
