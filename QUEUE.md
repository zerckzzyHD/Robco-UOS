...
**What it is.** A single save-contract hardening pass that came out of a blind completeness review (GPT + Gemini audited the roadmap for gaps; each finding was then verified against the real code before landing here). It's "the campaign data is safe" work, not visual polish, and it has two layers, not two separate items — they're one pass because they answer one question ("did the save survive?") from two different angles: does the app itself preserve every field, and does the platform actually keep the bytes around to preserve.
...
- **Layer 1 — semantic survival (the save-contract / upgrade-path health pass).** Nothing today proves a months-old save actually survives loading into a _newer_ version of the app. The v7→v8 migration path is tested, but only as key-mapping — not as "a real, fully-populated OLD save boots all the way to READY without silently losing a field." Silent field loss is exactly the class of bug fixed twice today (the cross-game registry-leak fix and its hardening guard, noted above in item 4). This pass verifies the existing coverage actually forms a complete contract and closes any gaps it finds, using real fixtures — a current save, a mature/high-density save, the oldest-still-supported save, a deliberately malformed one, and a save where the local copy and the cloud copy disagree — and it compares the durable FIELDS themselves rather than the raw saved text, so the test doesn't become brittle the next time a file gets reorganized. It also sets the fail-loud bar for the whole pass: a failed, interrupted, or quota-exhausted write must fail loudly with the original save left intact — never silently swallowed — because layer 2's storage request is a request, not a guarantee, and doesn't relieve the app of handling its own write errors. **Done means:** an automated check proves durable campaign data survives serialization, migration, an app-version update, an offline reload, a malformed input, and a cloud sync, with zero silent field loss anywhere in that chain, and every write failure in that chain surfaces instead of vanishing.

- **Layer 2 — storage survival (the persistent-storage request).** The app is mobile-primary, offline-first, and save-sacred — but it never once asks the browser to protect its data (`navigator.storage.persist()` is called nowhere in the codebase). iOS Safari in particular will quietly evict localStorage/IndexedDB under low storage pressure or after roughly two weeks unopened, which can silently erase a campaign while every test in the gate stays green. The fix: ask for persistent storage at boot — but requesting persistence is not the same as _proving_ the storage is durable, and the app must not treat the request as a guarantee. If the browser DENIES the request, the app is still volatile, and that DENIED case is what actually needs testing (not just that the request fires) — tell the user in-fiction when it happens (a "memory core unstable" style warning), rather than staying silent about the risk. Small effort. **Done means:** the app asks the browser to keep its data on every boot, the DENIED path is exercised by a real test (not just the happy-path request), and the terminal warns you, in its own voice, whenever the browser says no.
