...
- **The fourth context source — the `robco-uos` skill — finally has a tracked, guarded home (R9, SHIPPED 2026-07-21).** A session is oriented by four things: the rules, this queue, the memory store, and an installed **skill** that loads before the repo is even opened. Three had drift protection; the skill had none, no in-repo source, and had rotted — two sessions found it pointing at a dead repo path, citing the deleted PowerShell runner and a retired protocol. Now: a **committed source** (`skill/SKILL.md`) the gate fact-checks (**Suite 243** fails the build if it names a path/protocol that no longer exists), plus a self-clearing nudge for the one thing the gate can't see — the read-only _installed_ copy going stale (only you can re-install it, via Settings › Capabilities; **the nudge already flagged the stale install on this machine — re-install when convenient**). Decided _against_ generating it: a skill is editorial judgment about what matters most, so it stays hand-written but mechanically fact-checked — the same model the docs use. Full detail: R9 below.
- **Protocol 50 got a candid subsection (c) for the gap it can't close.** The queue-drift nudge catches _memory ↔ queue_ drift; it can't catch _conversation ↔ queue_ drift — proven within hours of shipping by the DeepSeek roster decision, which lived only in a conversation. The honest verdict, now in the protocol: a script can't read a conversation and the riskiest sessions never push, so this is **behavioural with no honest automated backstop**, fixed by a session-end ritual, not a fake guard. No new script was built for it — that would be the "guard that pretends" the retirement rule warns against.
...
### R9. ✅ SKILL SOURCE GUARD — the fourth context source finally gets a tracked, guarded home (SHIPPED 2026-07-21)

**What it is.** A session is oriented by four context sources: (1) `CLAUDE.md` + the `rules/*.md` notes, (2) this file, (3) the orchestrator's memory store, and (4) an installed **skill** (`robco-uos`) — a portable summary of the gate, workflow, branch model and data-safety rules that loads even before the repo is opened. The first three all grew drift protection this month; the fourth had **none**. It had no tracked source in the repo, so it could not be reviewed, diffed, or guarded, and every update was hand-retyped from memory — which is exactly how it rotted: **two separate sessions independently found the installed skill pointing at a dead repo path (`C:\Dev\!GEM\Website version`), citing the deleted PowerShell test runner and "both runners" parity, referencing retired Protocol 2a as live, and naming the deleted `RULES.md`.** Sessions either trusted it (bad) or noticed and ignored it (wasteful) — and it's injected as authoritative context.

**The decision — generate vs. hand-write, evaluated not assumed.** The owner's instinct was to generate the skill from the repo's own rules (the same "a hand-written description of a moving thing drifts" logic that drove every good call this week). Evaluated and **rejected in favour of a hybrid**: a skill is **editorial** — a judgement about what matters _most_, compressed — and full generation would either flatten that (a lossy extract still needs a human to choose what to extract, so the judgement just moves into harder-to-tune code) or balloon back toward reproducing `CLAUDE.md`, defeating the point of a portable summary. And this project has been burned by "generated-in-name-only" before (the test catalog). So: the skill stays **hand-written**, but every checkable FACT in it is **mechanically fact-checked** — the exact "hand-written prose, guarded facts" model Suite 220 already runs on the docs.

**Shipped:**

- **The corrected source, committed at `skill/SKILL.md`.** Every stale fact fixed: the current repo path, the dev→staging / main→prod branch model (Protocol 43), the THREE-model Fable/Opus/Sonnet hand-off (Protocol 8, not the old two-model track), the single Node runner (no PowerShell mirror, no test count tracked), semver auto-bump (Protocol 2), and a header stating the source is guarded and the installed copy is a read-only cache only the owner re-installs.
- **`Suite 243` — the guard.** Fails the build if `skill/SKILL.md` names a `js/`·`css/`·`tests/`·`scripts/`·`rules/` path that doesn't exist, references a "Protocol N" that resolves to no heading, reintroduces a known-stale claim (`!GEM`, `robco-diagnostics.ps1`, "both runners", `RULES.md`), or drifts its live URL from `README.md`'s — with a red-then-green proof that the detectors actually fire on a fabricated bad skill (non-vacuous). Reuses Suite 220's path-regex and rulebook-heading parse rather than re-inventing them (Protocol 22).
- **`scripts/skill-drift-check.js` — the nudge for the one axis the gate can't see.** The gate keeps the _source_ correct; it cannot see the _installed_ cache (read-only, per-machine, absent on CI). This fail-safe pre-push nudge (same `|| true` never-blocks shape as the backup and queue-drift nudges) flags when the installed skill still contains a phrase the corrected source has removed — **self-clearing** by construction (the instant the owner re-installs, the markers are gone and it falls silent, so it can never become a false alarm that fires forever). It fired correctly on this machine on first run, naming the real stale install.

**Why a nudge and not a gate for the install.** Nobody but the owner can write the skill cache — the desktop app installs skills via **Settings › Capabilities**. So the guard's honest scope is "the committed source is never internally stale"; re-installing is the owner's manual step. Unlike the conversation↔queue gap (R8 follow-up above), the skill's highest-risk action — editing the source — _does_ reach the pre-push hook, so the nudge fires where it matters.

**Also done:** the skill was added to the standing workflow-review prompt's §5 (governance/context), since it's part of how a session acquires context — squarely the review's subject.

**Done means:** met. `skill/SKILL.md` is committed and guarded (Suite 243, all green), the drift nudge is wired into `scripts/pre-push`, and the review prompt names the skill as the fourth context source. The one remaining manual step is the owner re-installing the corrected skill via Settings › Capabilities — the nudge will keep reminding until then.
