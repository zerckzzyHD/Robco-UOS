...
**What it depends on.** The archive repo and its folder structure (exists, Protocol 48) — the re-pin (R4) and ongoing sync keep it current. P1 (reproducibility) should finish, and ideally J (below) should exist and pass, before P2 (publication) is attempted — a publish-time diff check is only trustworthy if the thing being diffed reproduces the same way every time. ✅ **The App Check debug-token blocker on publication is CLEARED (2026-07-20)** — all three tokens were deleted in the Firebase console and the dead string is out of the archive. See the App Check entry in the 2.9.0 round.
