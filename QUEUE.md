...
- **The Fallout 3 device skin is IN PROGRESS** (2.8.5 item 4) — units U0-U8 have shipped on `dev` (the Pip-Boy spine, the weathered device casing, the six re-laid-out landscape boards, the scroll-trap fix, a real render-integrity guard that now checks 12 configs across both games, and — U8 — Fable's final Vault Boy figure plus the full U7 audit punch-list closed out). **A newly-approved save-integrity pass (item 5 below) now jumps ahead of what's left of FO3's cosmetic work** — a browser silently eating a campaign outranks an item list being one row short. After that: the deferred bottom-dock occlusion unit and the post-FO3 skin-architecture extraction pass (still item 4), then the legacy/schematic per-game layout (now item 6).
...
**Still owed on FO3, before it counts as "done" — now sequenced AFTER the new save-integrity pass (item 5 below):**
...
## 5. ⏭️ Save integrity pass (NEW, 2026-07-14 — jumps ahead of the remaining FO3 cosmetics)

**What it is.** Three small, related hardening items that came out of a blind completeness review (GPT + Gemini audited the roadmap for gaps; each finding was then verified against the real code before landing here). All three are "the campaign data is safe" work, not visual polish, and they're bundled because they share the same review and the same low effort.

**Why this jumps the queue.** It moves ahead of what's left of FO3's cosmetic work — the bottom-dock occlusion fix, the rest of the FO3 board polish, and the legacy/schematic per-game layout (item 6 below) — but **not** ahead of the karma rebuild or the data-provenance sweep, which are content-correctness work, not cosmetics, and stay exactly where they were. The reasoning in one line: a browser silently eating a campaign is worse than an item list being one row short.

- **a) Persistent-storage request.** The app is mobile-primary, offline-first, and save-sacred — but it never once asks the browser to protect its data (`navigator.storage.persist()` is called nowhere in the codebase). iOS Safari in particular will quietly evict localStorage/IndexedDB under low storage pressure or after roughly two weeks unopened, which can silently erase a campaign while every test in the gate stays green. The fix: ask for persistent storage at boot, and if the browser refuses, tell the user in-fiction (a "memory core unstable" style warning), rather than staying silent about the risk. Small effort. **Done means:** the app asks the browser to keep its data on every boot, and warns you, in the terminal's own voice, if the browser says no.

- **b) Save-contract / upgrade-path health pass.** Nothing today proves a months-old save actually survives loading into a _newer_ version of the app. The v7→v8 migration path is tested, but only as key-mapping — not as "a real, fully-populated OLD save boots all the way to READY without silently losing a field." Silent field loss is exactly the class of bug fixed twice today (the cross-game registry-leak fix and its hardening guard, noted above in item 4). This pass verifies the existing coverage actually forms a complete contract and closes any gaps it finds, using real fixtures — a current save, a mature/high-density save, the oldest-still-supported save, a deliberately malformed one, and a save where the local copy and the cloud copy disagree — and it compares the durable FIELDS themselves rather than the raw saved text, so the test doesn't become brittle the next time a file gets reorganized. **Done means:** an automated check proves durable campaign data survives serialization, migration, an app-version update, an offline reload, a malformed input, and a cloud sync, with zero silent field loss anywhere in that chain.

- **c) External-network / CDN chokepoint guard.** Nothing today stops someone from accidentally wiring a new external link into a core path — which would hang the UI on an offline, cache-cleared load. Today the only outside references are two intentional ones (Firebase's own CDN load inside the cloud module, which is online-only anyway, and the one user-facing "get your API key" link), so this is cheap to close now while the list is still short. The guard allowlists those two known references and fails the build the moment a new external `src`/`href`/`url()`/`import` shows up in a served file. **Done means:** a gate check stops a new networked dependency from silently breaking the offline load, before it ever ships.
