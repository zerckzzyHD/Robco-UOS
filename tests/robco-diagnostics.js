...
// ══════════════════════════════════════════════════════════════
{
  ...
  // 116.7  fire points wired at the key events — level-up + faction-threshold alert
  //        are RobcoEvents subscribers in ui-audio.js (U7); critical HP stays a
  //        RobcoEvents subscriber in ui-core.js (both were inline before U7)
  assert(
    /triggerHaptic\('levelup'\)/.test(uiAudio116) &&
...
//  auto-log expansion (Step 2 / v2.8.0 Phase 0)
//
//  U7 added a tiny synchronous pub/sub (RobcoEvents.on/emit, js/state.js) and
//  re-pointed the 3 previously-inline state-crossing detectors (level-up,
//  faction-threshold, HP-critical) to emit through it instead of each acting
//  inline. The faction-threshold detector's hardcoded FNV-only key array
//  (['ncr','legion','house','bos','boomers','khans']) — which meant FO3
...
//  (previously factions/quests/locations only) to level-ups, collectible
//  acquisitions, crafts, trades, and sleeps, routed through the same bus and
//  written via the single _logCampaignEvent() helper (state.js). Also locks the
//  Protocol 42 boot-order fix: each cross-file RobcoEvents.on() registration is
...
// ══════════════════════════════════════════════════════════════
{
  ...
  assert(
    /const RobcoEvents\s*=\s*\(\(\)\s*=>\s*\{/.test(stateSource) &&
      /function on\(event, fn\)/.test(stateSource) &&
      /function emit\(event, payload\)/.test(stateSource) &&
      /window\.RobcoEvents\s*=\s*RobcoEvents/.test(stateSource),
    '135.1: js/state.js declares RobcoEvents (on/emit) and exposes window.RobcoEvents'
  );

  // ── Behavioral bus tests (VM) ────────────────────────────────────────────
  const vm135 = require('vm');
  function makeBusSandbox135() {
    const sandbox = { window: {}, document: { getElementById: () => null } };
    vm135.createContext(sandbox);
    vm135.runInContext(stateSource, sandbox);
    return sandbox;
  }

  // 135.2  emit → subscribe fires: a single on() + a single emit() calls the
  //        handler exactly once with the emitted payload
  {
    let ok = false;
    try {
      const sb = makeBusSandbox135();
      sb.calls = [];
      vm135.runInContext("RobcoEvents.on('t.event', p => { calls.push(p); })", sb);
      vm135.runInContext("RobcoEvents.emit('t.event', { x: 1 })", sb);
      ok = sb.calls.length === 1 && sb.calls[0].x === 1;
    } catch (e) {
      fail(`135.2 harness error: ${e.message}`);
    }
    assert(ok, '135.2: RobcoEvents.on() + emit() calls the handler exactly once with the payload');
  }

  // 135.3  no double-fire: two emits call a single handler twice, not four times
  {
    let ok = false;
    try {
      const sb = makeBusSandbox135();
      sb.calls = [];
      vm135.runInContext("RobcoEvents.on('t.event', p => { calls.push(p); })", sb);
      vm135.runInContext("RobcoEvents.emit('t.event', 1)", sb);
      vm135.runInContext("RobcoEvents.emit('t.event', 2)", sb);
      ok = sb.calls.length === 2 && sb.calls[0] === 1 && sb.calls[1] === 2;
    } catch (e) {
      fail(`135.3 harness error: ${e.message}`);
    }
    assert(ok, '135.3: no double-fire — N emits call a single-registered handler exactly N times');
  }

  // 135.4  multiple independent subscribers to the same event each fire once per emit
  {
    let ok = false;
    try {
      const sb = makeBusSandbox135();
      sb.a = 0;
      sb.b = 0;
      vm135.runInContext("RobcoEvents.on('t.event', () => { a++; })", sb);
      vm135.runInContext("RobcoEvents.on('t.event', () => { b++; })", sb);
      vm135.runInContext("RobcoEvents.emit('t.event')", sb);
      ok = sb.a === 1 && sb.b === 1;
    } catch (e) {
      fail(`135.4 harness error: ${e.message}`);
    }
    assert(ok, '135.4: multiple subscribers to the same event each fire exactly once per emit');
  }

  // 135.5  a throwing handler is swallowed — does not break other handlers or escape emit()
  {
    let ok = false;
    try {
      const sb = makeBusSandbox135();
      sb.b = 0;
      vm135.runInContext("RobcoEvents.on('t.event', () => { throw new Error('boom'); })", sb);
      vm135.runInContext("RobcoEvents.on('t.event', () => { b++; })", sb);
      vm135.runInContext("RobcoEvents.emit('t.event')", sb);
      ok = sb.b === 1;
    } catch (e) {
      // ok stays false — a throw escaping emit() is exactly what must never happen
    }
    assert(
      ok,
  ...
  // 135.6  emitting an event with zero subscribers does not throw
  {
    let ok = false;
    try {
      const sb = makeBusSandbox135();
      vm135.runInContext("RobcoEvents.emit('nobody.listens', { x: 1 })", sb);
      ok = true;
    } catch (e) {
      // ok stays false
    }
    assert(ok, '135.6: emit() on an event with no subscribers does not throw');
  }

  // ── Faction-threshold Protocol-38 fix (static + behavioral) ──────────────

  // 135.7  negative guard: the old FNV-hardcoded faction-key array is gone
  //        (an assignment form, not just the identifier — so this doesn't
  //        false-positive on the explanatory comment describing the old code)
  assert(
    !/majorFactionKeys/.test(apiSource) && !/=\s*\['ncr',\s*'legion'/.test(apiSource),
  ...
  assert(
    /VILIFIED_NET[\s\S]{0,300}getFactionRegistry\(\)\.forEach/.test(apiSource),
    '135.8: the faction-threshold detector iterates getFactionRegistry() — game-agnostic, not a hardcoded key list'
  );

  // 135.9  behavioral: an FO3-only faction key (never in the old hardcoded FNV
  //        array) crossing the Vilified threshold fires 'faction.threshold' —
  //        proof the fix actually works for FO3, not just that the literal is gone
  {
    let ok = false;
    let err = null;
    try {
      ...
      vm135.createContext(sandbox);
      vm135.runInContext(stateSource, sandbox);
      const autoImportBody135 =
        'function autoImportState(jsonString)' + extractFunctionBody(apiSource, 'autoImportState');
      vm135.runInContext(autoImportBody135 + '\nthis.autoImportState = autoImportState;', sandbox);
      sandbox.captured = null;
      vm135.runInContext("RobcoEvents.on('faction.threshold', p => { captured = p; })", sandbox);
      // Talon Company ('talon') is FO3-only and was never in the old FNV array.
      vm135.runInContext(
        "state = { gameContext: 'FO3', ticks: 0, campaign_notes: [], factions: { talon: { fame: 0, infamy: 0 } } };",
        sandbox
      );
      sandbox.window._lastStateBeforeSync = JSON.stringify({
        factions: { talon: { fame: 0, infamy: 400 } }, // net -400 (above Vilified)
      });
      sandbox.autoImportState(
        JSON.stringify({ factions: { talon: { fame: 0, infamy: 600 } } }) // net -600 (crosses Vilified)
      );
      const captured = vm135.runInContext('captured', sandbox);
      ok = !!captured && captured.key === 'talon' && captured.direction === 'vilified';
    } catch (e) {
      err = e;
    }
    assert(
  ...
  assert(
    /RobcoEvents\.emit\(\s*'hp\.critical'/.test(uiCoreSrc135),
  ...
  assert(
    [
      ...
      new RegExp(`RobcoEvents\\.on\\(\\s*'${ev.replace('.', '\\.')}'`).test(stateSource)
    ),
  ...
  // Discovered live: ui-audio.js/ui-core.js/api.js are static <script> tags that
  // CAN execute before state.js (which defines RobcoEvents) finishes its dynamic,
  // context-conditional load (js/db_nv.js / state.js / js/reg_nv.js / registry-core.js
  // are injected via document.createElement('script'), per the boot-loader comment
  // in index.html — "Static ui-*/api/cloud tags below ... may parse first"). A
  // bare top-level RobcoEvents.on(...) call in any of those 3 files threw
  // "RobcoEvents is not defined" on some boots (confirmed via the Playwright
  // boot-smoke check, a real shipped-code path — not a harness artifact). Fixed
  ...
  //         named wiring function, and window.onload calls all three
  {
    ...
    if (olIdx135 !== -1) {
      ...
      let depth135 = 0,
        i135 = braceStart135;
      for (; i135 < uiCoreSrc135.length; i135++) {
        if (uiCoreSrc135[i135] === '{') depth135++;
        else if (uiCoreSrc135[i135] === '}' && --depth135 === 0) break;
      }
      onloadBody135 = uiCoreSrc135.slice(braceStart135, i135 + 1);
    }
    assert(
      /function _wireAudioEventBusSubscribers\(\)\s*\{[\s\S]*?RobcoEvents\.on\(/.test(
        uiAudioSrc135
      ) &&
        /function _wireCoreEventBusSubscribers\(\)\s*\{[\s\S]*?RobcoEvents\.on\(/.test(
          uiCoreSrc135
        ) &&
        /function _wireApiEventBusSubscribers\(\)\s*\{[\s\S]*?RobcoEvents\.on\(/.test(apiSource) &&
        [
          '_wireCoreEventBusSubscribers',
          '_wireAudioEventBusSubscribers',
          '_wireApiEventBusSubscribers',
        ].every(n => onloadBody135.includes(n + '()')),
  ...
  //         registration has crept back in (the exact shape of the boot-order bug)
  {
    ...
    assert(
      audioCheck135.total === audioCheck135.inside &&
        coreCheck135.total === coreCheck135.inside &&
        apiCheck135.total === apiCheck135.inside &&
...
//  balanced), standby-deepen (tier balanced, layered OVER the A2 essential
//  'minimal'-tier dim, unchanged), and shutdown-crt (tier full, degrading to
//  a plain instant cut below Full). onEnter/onExit are NOT tier-gated by the
//  runtime itself (only onTick is — see runtime.js's _beat()), so each
//  observer's onEnter checks immersionAllows() itself before toggling its
//  body class; onExit always cleans up unconditionally (no leaked class).
...
// ══════════════════════════════════════════════════════════════
{
  ...
  assert(
    /_wireStandby\(\);[\s\S]*?_wireAmbientExperiences\(\);[\s\S]*?initAmbientRuntime\(\);/.test(
      uiCore150
    ),
  ...
  assert(
    /id:\s*'idle-phosphor'/.test(wireAmbient150) &&
      /states:\s*\['IDLE'\]/.test(wireAmbient150) &&
      /tier:\s*'balanced'/.test(wireAmbient150) &&
      /if \(typeof immersionAllows === 'function' && !immersionAllows\('balanced'\)\) return;\s*\n\s*document\.body\.classList\.add\('rt-idle'\);/.test(
        wireAmbient150
      ) &&
      /onExit:\s*\(\)\s*=>\s*document\.body\.classList\.remove\('rt-idle'\)/.test(wireAmbient150),
  ...
  assert(
    /id:\s*'standby-deepen'/.test(wireAmbient150) &&
      /states:\s*\['STANDBY'\]/.test(wireAmbient150) &&
      (wireAmbient150.match(/tier:\s*'balanced'/g) || []).length === 2 &&
      /document\.body\.classList\.add\('standby-deep'\)/.test(wireAmbient150) &&
      /onExit:\s*\(\)\s*=>\s*document\.body\.classList\.remove\('standby-deep'\)/.test(
        wireAmbient150
      ),
  ...
  assert(
    /id:\s*'shutdown-crt'/.test(wireAmbient150) &&
      /states:\s*\['SHUTDOWN',\s*'OFF'\]/.test(wireAmbient150) &&
      /tier:\s*'full'/.test(wireAmbient150) &&
      /classList\.remove\('rt-idle', 'standby-deep', 'standby'\)/.test(wireAmbient150) &&
      /classList\.add\(full \? 'rt-shutdown' : 'rt-shutdown-plain'\)/.test(wireAmbient150) &&
      /classList\.remove\('rt-shutdown', 'rt-shutdown-plain'\)/.test(wireAmbient150),
  ...
  assert(
    /body\.rt-idle \.container/.test(css150) &&
      /body\.rt-idle::after/.test(css150) &&
      /body\.standby-deep::before/.test(css150) &&
      !/body\.standby-deep::after/.test(css150) &&
      /body\.rt-shutdown \.container/.test(css150) &&
      /body\.rt-shutdown::after,\s*\n\s*body\.rt-shutdown-plain::after/.test(css150) &&
      /#bootScreen\s*\{[\s\S]*?z-index:\s*100000;/.test(css150) &&
      /body\.rt-shutdown::after,\s*\n\s*body\.rt-shutdown-plain::after\s*\{[\s\S]*?z-index:\s*100001;/.test(
        css150
      ),
  ...
  //        none is needed)
  assert(
    /@keyframes idle-breathe/.test(css150) &&
      /@keyframes standby-breathe/.test(css150) &&
      /@keyframes crt-power-off/.test(css150) &&
      /@keyframes shutdown-cover \{/.test(css150) &&
      /@keyframes shutdown-cover-plain/.test(css150) &&
      /animation: idle-breathe/.test(css150) &&
      /animation: standby-breathe/.test(css150) &&
      /animation: crt-power-off/.test(css150) &&
      /animation: shutdown-cover /.test(css150) &&
      /animation: shutdown-cover-plain/.test(css150),
...
// ══════════════════════════════════════════════════════════════
{
  ...
  assert(
    /<div class="chip-grid" id="chipGrid">/.test(html156) &&
      /\.sonic-board--ejected \.chip-card \{[\s\S]{0,120}(border-style:\s*dashed|opacity:\s*0\.5)/.test(
        css156
      ) &&
      /\.sonic-board--ejected \.chip-card \.pin-id::after \{[\s\S]{0,60}content:\s*' · SOCKET EMPTY'/.test(
        css156
      ),
  ...
  const sonicStatusFn156 = extractFunctionBody(audio156, '_updateSonicBoardStatus');
  assert(
    /classList\.toggle\('sonic-board--ejected', !!AudioSettings\.masterMute\)/.test(
      sonicStatusFn156
    ) && !/MetaStore\.set/.test(sonicStatusFn156),
  ...
  const toggleAudioFn156 = extractFunctionBody(audio156, 'toggleAudio');
  assert(
    /MetaStore\.set\(key, isMuted\)/.test(toggleAudioFn156),
  ...
  const cycleFn156 = extractFunctionBody(core156, '_cycleImmersionDial');
  assert(
    /const IMMERSION_ORDER = \['full', 'balanced', 'minimal'\];/.test(core156) &&
      /let _dialDragSuppressClick = false;/.test(core156) &&
      /if \(_dialDragSuppressClick\) \{\s*_dialDragSuppressClick = false;\s*return;/.test(
        cycleFn156
      ) &&
      /onImmersionChange\(next\)/.test(cycleFn156),
  ...
  const dialMoveFn156 = extractFunctionBody(core156, '_dialPointerMove');
  const wireDragFn156 = extractFunctionBody(core156, '_wireImmersionDialDrag');
  assert(
    /onImmersionChange\(IMMERSION_ORDER\[idx\]\)/.test(dialMoveFn156) &&
      /typeof window\.PointerEvent === 'undefined'/.test(wireDragFn156) &&
      /addEventListener\('pointerdown', _dialPointerDown\)/.test(wireDragFn156) &&
      /_wireImmersionDialDrag\(\);/.test(extractFunctionBody(core156, 'initImmersion')),
  ...
  //        untouched
  assert(
    /button\.dial \{[\s\S]{0,700}touch-action:\s*none/.test(css156) &&
      /class="dial"[\s\S]{0,60}onclick="_cycleImmersionDial\(\)"/.test(html156),
  ...
  assert(
    /robco_hardwaresfx_muted: \{ type: 'bool', default: false, owner: 'ui-audio\.js' \}/.test(
      state156
    ) && /hardwareSfx: MetaStore\.get\('robco_hardwaresfx_muted'\) === 'true'/.test(core156),
  ...
  const chipClickFn156 = extractFunctionBody(audio156, 'playChipClick');
  const boardThunkFn156 = extractFunctionBody(audio156, 'playBoardThunk');
  const guardOrderOk = fn =>
    /if \(AudioSettings\.masterMute\) return;\s*\n\s*if \(AudioSettings\.hardwareSfx\) return;/.test(
      fn
    );
  assert(
    guardOrderOk(chipClickFn156) && guardOrderOk(boardThunkFn156),
  ...
  assert(
    /robco_hardwaresfx_muted: 'hardwareSfx', \/\/ B2c/.test(toggleAudioFn156) &&
      /playChipClick\(!isMuted\);/.test(toggleAudioFn156),
  ...
  assert(
    /playChipClick\(on\);/.test(extractFunctionBody(audio156, 'toggleRadio')) &&
      /playChipClick\(enabled\);/.test(extractFunctionBody(core156, 'toggleWakeLock')) &&
      /playChipClick\(enabled\);/.test(extractFunctionBody(audio156, 'toggleHaptic')),
  ...
  const audioPanel156 = html156.slice(
    html156.indexOf('id="audioSystemsDetails"'),
    html156.indexOf('id="chipGrid"')
  );
  assert(
    /id="hardwareSfxToggle"[\s\S]{0,40}class="bay-module-input"[\s\S]{0,40}checked[\s\S]{0,40}onchange="toggleAudio\('robco_hardwaresfx_muted', !this\.checked\)"/.test(
      audioPanel156
    ) && /SERVO CLICK RELAY/.test(audioPanel156),
  ...
  assert(
    /if \(MetaStore\.get\('robco_hardwaresfx_muted'\) === 'true'\) \{\s*\n\s*let el = document\.getElementById\('hardwareSfxToggle'\);\s*\n\s*if \(el\) el\.checked = false;/.test(
      core156
    ) &&
      /hwSfxToggle\.checked = MetaStore\.get\('robco_hardwaresfx_muted'\) !== 'true'/.test(
        core156
      ) &&
      /function _schemSetHardwareSfx\(checked\) \{\s*\n\s*toggleAudio\('robco_hardwaresfx_muted', !checked\);/.test(
        core156
      ) &&
      /checkboxRow\('_schemSetHardwareSfx', hwSfxOn, 'SERVO CLICK RELAY', 'SLOT 02'\)/.test(
        core156
      ),
  ...
  assert(
    /`robco_hardwaresfx_muted`\s*\|\s*bool\s*\|\s*ui-audio\.js/.test(arch156),
  ...
  const b2cJs156 =
    ...
    sonicStatusFn156;
  assert(
    !/\bFNV\b|\bFO3\b|Fallout/.test(b2cJs156) && !/\bFNV\b|\bFO3\b|Fallout/.test(audioPanel156),
  ...
  assert(
    !/<span[^>]*onclick=/.test(html156),
    '156.16: no <span onclick> pattern anywhere (Protocol UI-5) after the B2c additions'
  );
}
...
// ══════════════════════════════════════════════════════════════
{
  ...
  assert(
    /class="composer-icon-btn"\s+onclick="triggerImageUpload\(\)"/.test(composerBlock162) &&
      /id="modePill"[\s\S]{0,200}onclick="toggleInputMode\(\)"/.test(composerBlock162) &&
      /class="composer-icon-btn composer-help-btn"\s+onclick="showHelpModal\(\)"/.test(
        composerBlock162
      ) &&
      /onclick="submitCommandInput\(\)"\s+class="composer-send-btn"\s+id="transmitBtn"/.test(
        composerBlock162
      ),
  ...
  //         aria-label (fits the small circular button) instead of the old
  //         long button-text strings, and the Protocol 42 fix closes the
  //         onclick-drift bug found while adapting this button: the finally
  //         block used to rebind onclick straight to transmitMessage(),
  //         permanently bypassing submitCommandInput()'s TERMINAL-mode/
